...
function IdleOverlay({ onStart, onExit, sw, sh }) {
  ...
  useEffect(() => {
    const loadLaser = async () => {
      ...
      // In the existing sound useEffect, ADD bg music loading after the SFX loop:
      try {
        const { sound } = await Audio.Sound.createAsync(
          require("../../assets/sounds/game/super-hero-mission/bg.mp3"),
        ...
        >
          ...
          <View style={st.roundBadge}>
            ...
            <Text style={st.roundBadgeText}>
              Mission {round.id} of {totalRounds}
            </Text>
          </View>
...
export default function SuperHeroMissionGame({ onExit }) {
  ...
  const rounds = useMemo(() => {
    try {
      ...
      if (
        ...
        console.log(
          "[SuperHeroMissionGame] using backend rounds, count:",
          parsed.length,
        );
        return parsed;
      }
  ...
  const sndBg = useRef(null); // ← must be here, before startRound/endRound/handleExit
