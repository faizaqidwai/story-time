 ...
 * Word Opposites game with pirate theme.
 *
 * Assets:
 *   assets/games/spinwheel/parrot_idle.png
 *   assets/games/spinwheel/parrot_spin.png
 *   assets/games/spinwheel/parrot_correct.png
 *   assets/games/spinwheel/parrot_wrong.png
 *   assets/games/spinwheel/ship_bg.jpg
 *   assets/games/spinwheel/wheel_center.png
 *   assets/sounds/game/spinwheel/spin.mp3
 *   assets/sounds/game/spinwheel/tick.mp3
 *   assets/sounds/game/spinwheel/correct.mp3
 *   assets/sounds/game/spinwheel/wrong.mp3
 *   assets/sounds/game/spinwheel/win.mp3
 *   assets/sounds/game/spinwheel/start.mp3
 *   assets/sounds/game/spinwheel/pirate-start.mp3
    ...
    pairs: [
      {
        word: "HAPPY",
        opposite: "SAD",
        distractors: ["MAD", "TIRED", "BORED"],
      },
      {
        word: "CLEAN",
        opposite: "DIRTY",
        distractors: ["SHINY", "DUSTY", "MESSY"],
      },
      {
        word: "BRAVE",
        opposite: "SCARED",
        distractors: ["STRONG", "PROUD", "SILLY"],
      },
    ],
  },
  {
    ...
    pairs: [
      {
        word: "ANCIENT",
        opposite: "MODERN",
        distractors: ["BROKEN", "RUSTY", "HEAVY"],
      },
      {
        word: "EXPAND",
        opposite: "SHRINK",
        distractors: ["STRETCH", "FLATTEN", "TWIST"],
      },
      {
        word: "GENEROUS",
        opposite: "GREEDY",
        distractors: ["NERVOUS", "CAREFUL", "HONEST"],
      },
    ],
  },
  {
    ...
    pairs: [
      {
        ...
        distractors: ["REALISTIC", "ENERGETIC", "CREATIVE"],
      },
      {
        word: "ACCELERATE",
        opposite: "DECELERATE",
        distractors: ["NAVIGATE", "EVAPORATE", "CELEBRATE"],
      },
      {
        word: "CONSTRUCT",
        opposite: "DEMOLISH",
        distractors: ["ACCOMPLISH", "DISTINGUISH", "ESTABLISH"],
      },
    ],
  },
];
...
// 8 wheel segments: question word at slot 0, 7 random fillers
function makeSegments(questionWord) {
  const fillers = shuffle(WHEEL_POOL.filter((w) => w !== questionWord)).slice(
    0,
    7,
  );
...
// ─────────────────────────────────────────────────────────────
// SPIN WHEEL
// 4 spokes at 0°,45°,90°,135° → 8 sectors of 45° each
// Words placed at the MIDPOINT of each sector (between spokes)
// ─────────────────────────────────────────────────────────────
function SpinWheel({ rotateAnim, segments, revealed }) {
  ...
  // Glow pulse for the question word slot (index 0) when revealed
  ...
  return (
    ...
    >
      ...
      >
        ...
        {/* 4 SPOKES — each is a rectangle rotated through the center */}
        ...
        {/* 8 WORDS — one per sector, centred between the two bounding spokes */}
        {segments.map((word, i) => {
          ...
          const isQ = i === 0 && revealed; // only highlight AFTER reveal
          ...
          const wordColor = isQ
            ...
            : C.goldLight;
          const wordSize = isQ ? font.md : font.sm;
        ...
        >
          <Image
            source={require("../../assets/games/spinwheel/wheel_center.png")}
...
function AnswerChip({ word, state, onPress, disabled }) {
  ...
  return (
    <Animated.View
      style={{
        flex: 1,
        transform: [{ scale: scaleA }, { translateX: shakeA }],
      }}
    >
      <TouchableOpacity
        onPress={handlePress}
...
function DiffStars({ level }) {
  return (
    <View style={{ flexDirection: "row", gap: 3 }}>
      {[1, 2, 3, 4].map((i) => (
        <Text key={i} style={{ fontSize: 12, opacity: i <= level ? 1 : 0.2 }}>
          ⚓
        </Text>
      ))}
    </View>
...
export default function SpinWheelGame() {
  ...
  const [revealed, setRevealed] = useState(false); // true after word glow + speech
  ...
  // ── Intro sound sequence (stoppable)
  useEffect(() => {
    let cancelled = false;
    const t = setTimeout(async () => {
      ...
      // Wait up to 5s for pirate-start to finish
  ...
  const removeStar = (id) =>
    setFloatingStars((p) => p.filter((s) => s.id !== id));
  ...
  const handleSpin = () => {
    ...
    // Sector 0 (question word) is centred at 22.5° past the top.
    // The pointer sits at 0°. To land the word CENTRE under the pointer
    // we need the wheel to stop at exactly -22.5° offset from a full turn,
    // i.e. the fractional part of totalRot should be (360-22.5)/360 = 0.9375
    // so that sector-0 midpoint faces straight up.
    const currentFrac = totalRot.current % 1;
    const targetFrac = (360 - 22.5) / 360; // 0.9375
    let extraFrac = targetFrac - currentFrac;
    if (extraFrac <= 0) extraFrac += 1; // always move forward
  ...
  const handleAnswer = (word) => {
    ...
    if (correct) {
      ...
      playSound(SFX.wrong);
      setTimeout(() => {
        setAnswerStates({ [word]: "wrong", [pair.opposite]: "correct" });
      }, 600);
    }
  ...
  if (screen === "start") {
    return (
      <ImageBackground
        source={require("../../assets/games/spinwheel/ship_bg.jpg")}
        style={styles.root}
        resizeMode="cover"
      >
        ...
        <SafeAreaView style={styles.startWrap}>
          ...
          <Text style={styles.subtitle}>
            Spin the wheel — find the OPPOSITE!
          </Text>
        ...
        {floatingStars.map((s) => (
          <FloatingStar key={s.id} {...s} onDone={() => removeStar(s.id)} />
        ))}
      </ImageBackground>
    );
  }
          ...
          <View style={styles.dots}>
            {round.pairs.map((_, i) => (
              <View
                ...
                style={[
                  styles.dot,
                  i < pairIndex && styles.dotDone,
                  i === pairIndex && styles.dotActive,
                ]}
              />
            ))}
          </View>
            ...
            <View style={styles.bubble}>
              ...
              {!hasSpun && !spinning && (
                <Text style={styles.bubbleLabel}>
                  🎡 {pairIndex === 0 ? "Spin to begin!" : "Spin again!"}
                </Text>
              )}
          ...
          <View style={styles.wheelWrap}>
            <SpinWheel
              ...
              segments={
                segments.length === 8
                  ? segments
                  : makeSegments(pair?.word ?? "?")
              }
              revealed={revealed}
            />
          </View>

          {/* BOTTOM — spin btn or answer chips */}
          <View style={styles.bottom}>
            {!hasSpun ? (
              <TouchableOpacity
                style={[styles.goldBtn, spinning && styles.goldBtnDisabled]}
                onPress={handleSpin}
                disabled={spinning}
                activeOpacity={0.85}
              >
...
function RoundResultCard({
  ...
  return (
    <Animated.View
      style={[
        styles.card,
        { borderColor: aborder, opacity: op, transform: [{ scale: sc }] },
      ]}
    >
      <View
        style={[styles.banner, { backgroundColor: adim, borderColor: aborder }]}
      >
        {isWin && (
          <Animated.View
            pointerEvents="none"
        ...
        <Text style={[styles.bannerTxt, { color: ac }]}>
          {isWin ? `🦜 Round ${roundIndex + 1} Complete!` : "💪 Keep Sailing!"}
        </Text>
      ...
      <View style={styles.pillRow}>
        ...
        >
          ...
          <Text style={[styles.pillVal, { color: C.green }]}>
            {correct}/{total}
          </Text>
        </View>
        <View
      ...
      <TouchableOpacity
        style={[styles.goldBtn, { marginTop: 4 }]}
        onPress={onNext}
        activeOpacity={0.85}
      >
        <Text style={styles.goldBtnText}>
    ...
    >
      ...
      <View style={styles.starsRow}>
        {stars.map((s, i) => (
          <Animated.Text
            ...
            style={[
              styles.starEmoji,
              { transform: [{ scale: s }], opacity: i < filled ? s : 0.18 },
            ]}
          >
            ⭐
          </Animated.Text>
      ...
      <View style={styles.pillRow}>
        ...
        >
          ...
          <Text style={[styles.pillVal, { color: C.teal }]}>
            ⭐ {totalStars}/{maxStars}
          </Text>
        </View>
      </View>
        ...
        {roundScores.map((s, i) => (
          <View key={i} style={styles.breakdownRow}>
            <Text style={styles.breakdownLabel}>
              ⚓ Round {i + 1} — {ROUNDS[i].label}
            </Text>
            <View style={{ flexDirection: "row", gap: 3 }}>
              {[1, 2, 3].map((n) => (
                <Text
                  key={n}
                  style={{ fontSize: 13, opacity: n <= s ? 1 : 0.18 }}
                >
                  ⭐
                </Text>
              ))}
