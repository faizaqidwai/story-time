 ...
 *   ✅ Added splash screen with cover.jpg background + play.png button
 *   ✅ BG music loaded on mount with shouldPlay:true, isLooping:true, volume:0.2
 *   ✅ BG music pauses on result/final screens, resumes on playing
 *   ✅ All other game logic, wheel physics, animations, and UI unchanged
 */
...
// SPLASH SCREEN
// ─────────────────────────────────────────────────────────────
function SplashScreen({ onPlay, onClose }) {
  ...
  return (
    <ImageBackground
      source={require("../../assets/games/spinwheel/cover.png")}
    ...
    >
      ...
      <TouchableOpacity
        style={styles.splashExit}
        onPress={onClose}
        activeOpacity={0.8}
        hitSlop={{ top: 14, bottom: 14, left: 14, right: 14 }}
      >
        <Text style={styles.splashExitTxt}>✕</Text>
      </TouchableOpacity>
      ...
      <View style={styles.splashBottom}>
        <Animated.View style={{ transform: [{ scale: pulse }] }}>
          <TouchableOpacity onPress={onPlay} activeOpacity={0.85}>
            <Image
              source={require("../../assets/games/spinwheel/play.png")}
              style={styles.splashPlayImg}
              resizeMode="contain"
            />
          </TouchableOpacity>
        </Animated.View>
...
export default function SpinWheelGame() {
  ...
  const rounds = useMemo(() => {
    try {
      ...
      if (
        ...
        console.log(
          "[SpinWheelGame] using backend rounds, count:",
          parsed.length,
        );
        return parsed;
      }
  ...
  // ── State ─────────────────────────────────────────────────────────────────
  // "splash" → new initial screen; "start" → existing rules screen; "playing" etc.
  ...
  // ── Load BG music on mount — shouldPlay:true, loop, vol 0.2 ───────────────
  useEffect(() => {
    let alive = true;
    (async () => {
      try {
    ...
    setTimeout(
      () => {
        ...
        if (nextPair < round.pairs.length) {
          ...
          // Pause bg music on result screen
          sndBgRef.current?.pauseAsync().catch(() => {});
  ...
  const handleNextRound = () => {
    ...
    if (next < totalRounds) {
      ...
      setAnswerStates({});
      setParrot("idle");
      // Resume bg music when returning to playing
      sndBgRef.current
        ?.setPositionAsync(0)
        .then(() => sndBgRef.current?.playAsync())
        .catch(() => {});
      setScreen("playing");
    } else {
      // Pause bg music on final screen
      sndBgRef.current?.pauseAsync().catch(() => {});
      setScreen("final");
    }
  };
        ...
        {roundScores.map((s, i) => (
          <View key={i} style={styles.breakdownRow}>
            <Text style={styles.breakdownLabel}>
              ⚓ Round {i + 1} — {rounds[i]?.label ?? `Round ${i + 1}`}
            </Text>
            <View style={{ flexDirection: "row", gap: 3 }}>
              {[1, 2, 3].map((n) => (
                <Text
...
const styles = StyleSheet.create({
  ...
  splashExitTxt: { color: "#ECEFF1", fontSize: 15, fontWeight: "700" },
  splashBottom: {
    position: "absolute",
    bottom: "8%",
    left: 0,
