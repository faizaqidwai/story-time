...
function makeBgObj(type) {
  ...
  return {
    ...
    y: maxY - (isCloud ? Math.random() * (maxY - (STATUS_H + 20)) : h),
    w,
    h,
...
export default function FlappyWordGame({ onExit }) {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      // BG music — shouldPlay:true starts immediately on mount, same as DinoWorldGame
      try {
        const { sound } = await Audio.Sound.createAsync(
          require("../../assets/sounds/game/flappy-word/bg.mp3"),
          { shouldPlay: true, isLooping: true, volume: 0.6 },
        );
        if (alive) sndBg.current = sound;
        else sound.unloadAsync();
      } catch (e) {
        console.log("[FlappyWordGame] bg music error:", e);
      }
    })();
    return () => {
      alive = false;
      [sndButton, sndPop, sndCorrect, sndWrong, sndWin, sndLose, sndBg].forEach(
        (r) => {
  ...
  const handleExit = useCallback(() => {
    stopFlap();
    // Pause — music stays loaded for next time
  ...
  const initGame = useCallback(() => {
    ...
    setPhase("playing");
    startFlap();
    // Resume the already-loaded bg music from the beginning
    sndBg.current
      ?.setPositionAsync(0)
  ...
  const tick = useCallback(() => {
    ...
    const bx1 = BIRD_X - BIRD_W / 2 + 6,
      bx2 = BIRD_X + BIRD_W / 2 - 6;
    const by1 = birdYRef.current + 4,
      by2 = birdYRef.current + BIRD_H - 4;
    let died = false;
