 ...
 *   ✅ RoundBriefing receives totalRounds as prop
 *   ✅ Phase-driven TTS effect reads from rounds[roundIndex] (not ROUNDS[roundIndex])
 *   ✅ startRound reads from rounds (in deps), not ROUNDS
...
const FALLBACK_ROUNDS = [
  {
    ...
    briefing: 'Collect SIZE words to speed up!\n"big","tall","huge" help you catch the car.',
  ...
  },
  {
    ...
    briefing: 'Collect COLOR words to speed up!\n"red","blue","green" fuel the chase.',
  ...
  },
  {
    ...
    briefing: "Collect ANIMAL words to speed up!\nDog, cat, lion — animal names.",
  ...
  },
  {
    ...
    briefing: 'Collect FEELING words to speed up!\n"happy","brave","calm" fuel the chase.',
  ...
  },
  {
    ...
    briefing: 'Collect ACTION words to speed up!\n"run","jump","fly" — action words.',
...
function getLaneCentres(sw) {
  ...
  return Array.from({ length: NUM_LANES }, (_, i) => ROAD_PADDING + i * laneW + laneW / 2);
}

function makeWord(sw, sh, item) {
  const centres = getLaneCentres(sw);
  const laneW = (sw - ROAD_PADDING * 2) / NUM_LANES;
  const w = Math.min(item.word.length * WORD_CHAR_W + WORD_PAD_H * 2, laneW * 0.85);
  const cx = centres[Math.floor(Math.random() * NUM_LANES)];
  return {
    id: uid(), text: item.word, isTarget: item.isTarget,
    x: cx - w / 2, y: HEADER_H - WORD_H - 10, w, h: WORD_H, hit: false,
  };
}
...
function Road({ sw, sh, dashOffset }) {
  ...
  return (
    <View style={[StyleSheet.absoluteFill, { top: HEADER_H }]} pointerEvents="none">
      <View style={{ ...StyleSheet.absoluteFillObject, backgroundColor: C.road }} />
      <View style={{ position: "absolute", left: ROAD_PADDING, top: 0, bottom: 0, width: 4, backgroundColor: C.roadEdge, borderRadius: 2 }} />
      <View style={{ position: "absolute", right: ROAD_PADDING, top: 0, bottom: 0, width: 4, backgroundColor: C.roadEdge, borderRadius: 2 }} />
      {dividers.map((lx, li) => (
        <View key={li} style={{ position: "absolute", left: lx, top: 0, bottom: 0, width: 2, overflow: "hidden" }}>
      ...
      <View style={{ position: "absolute", left: 0, top: 0, bottom: 0, width: ROAD_PADDING, backgroundColor: "rgba(34,85,34,0.35)" }} />
      <View style={{ position: "absolute", right: 0, top: 0, bottom: 0, width: ROAD_PADDING, backgroundColor: "rgba(34,85,34,0.35)" }} />
    </View>
  );
}
...
function PoliceCar({ x, y, sirenAnim }) {
  ...
  const blueOp = sirenAnim.interpolate({ inputRange: [0, 1], outputRange: [1, 0] });
  return (
    <View pointerEvents="none" style={{ position: "absolute", left: x, top: y, width: POLICE_W, height: POLICE_H, zIndex: 70 }}>
      <View style={{ position: "absolute", left: 0, top: 0, width: POLICE_W, height: POLICE_H, backgroundColor: C.police, borderRadius: 8, borderWidth: 2, borderColor: "#0D47A1", shadowColor: C.teal, shadowOffset: { width: 0, height: 0 }, shadowOpacity: 0.6, shadowRadius: 8, elevation: 8 }}>
        <View style={{ position: "absolute", left: 0, right: 0, top: POLICE_H * 0.38, height: 6, backgroundColor: C.policeStripe, opacity: 0.9 }} />
        <View style={{ position: "absolute", left: 6, top: 6, right: 6, height: 16, backgroundColor: "rgba(144,200,255,0.6)", borderRadius: 4, borderWidth: 1, borderColor: "rgba(255,255,255,0.3)" }} />
        <View style={{ position: "absolute", left: 8, bottom: 8, right: 8, height: 12, backgroundColor: "rgba(144,200,255,0.5)", borderRadius: 3 }} />
        {[{ top: 8, left: -5 }, { top: 8, right: -5 }, { bottom: 8, left: -5 }, { bottom: 8, right: -5 }].map((pos, i) => (
          <View key={i} style={{ position: "absolute", ...pos, width: 8, height: 14, borderRadius: 3, backgroundColor: "#222", borderWidth: 1, borderColor: "#555" }} />
        ))}
      </View>
      <View style={{ position: "absolute", left: POLICE_W * 0.1, top: POLICE_H * 0.16, width: POLICE_W * 0.8, height: 10, borderRadius: 5, backgroundColor: "#111" }} />
      <Animated.View style={{ position: "absolute", left: POLICE_W * 0.1, top: POLICE_H * 0.16, width: POLICE_W * 0.38, height: 10, borderRadius: 5, backgroundColor: C.siren1, opacity: redOp, shadowColor: C.siren1, shadowOffset: { width: 0, height: 0 }, shadowOpacity: 1, shadowRadius: 8, elevation: 12 }} />
      <Animated.View style={{ position: "absolute", left: POLICE_W * 0.52, top: POLICE_H * 0.16, width: POLICE_W * 0.38, height: 10, borderRadius: 5, backgroundColor: C.siren2, opacity: blueOp, shadowColor: C.siren2, shadowOffset: { width: 0, height: 0 }, shadowOpacity: 1, shadowRadius: 8, elevation: 12 }} />
    </View>
  );
}
...
function BlackCar({ x, y }) {
  return (
    <View pointerEvents="none" style={{ position: "absolute", left: x, top: y, width: BLACK_W, height: BLACK_H, zIndex: 65 }}>
      <View style={{ position: "absolute", left: 0, top: 0, width: BLACK_W, height: BLACK_H, backgroundColor: C.blackCar, borderRadius: 7, borderWidth: 2, borderColor: "#444", shadowColor: "#000", shadowOffset: { width: 0, height: 3 }, shadowOpacity: 0.7, shadowRadius: 6, elevation: 8 }}>
        <View style={{ position: "absolute", left: 5, top: 5, right: 5, height: 14, backgroundColor: "rgba(100,180,255,0.4)", borderRadius: 3 }} />
        <View style={{ position: "absolute", left: 7, bottom: 7, right: 7, height: 11, backgroundColor: "rgba(100,180,255,0.3)", borderRadius: 3 }} />
        <View style={{ position: "absolute", bottom: 2, left: 3, width: 8, height: 5, backgroundColor: "#FF1744", borderRadius: 2 }} />
        <View style={{ position: "absolute", bottom: 2, right: 3, width: 8, height: 5, backgroundColor: "#FF1744", borderRadius: 2 }} />
        {[{ t: 6, l: -5 }, { t: 6, r: -5 }, { b: 6, l: -5 }, { b: 6, r: -5 }].map((pos, i) => (
          <View key={i} style={{ position: "absolute", ...pos, width: 7, height: 13, borderRadius: 3, backgroundColor: "#111", borderWidth: 1, borderColor: "#444" }} />
        ))}
      </View>
    </View>
  );
...
function WordChip({ word }) {
  ...
  const borderColor = glow.interpolate({ inputRange: [0, 1], outputRange: word.isTarget ? ["rgba(76,175,80,0.5)", "rgba(76,175,80,1)"] : ["rgba(239,83,80,0.4)", "rgba(239,83,80,0.9)"] });
  const bgColor = glow.interpolate({ inputRange: [0, 1], outputRange: word.isTarget ? ["rgba(76,175,80,0.12)", "rgba(76,175,80,0.28)"] : ["rgba(239,83,80,0.10)", "rgba(239,83,80,0.22)"] });

  return (
    <Animated.View pointerEvents="none" style={{ position: "absolute", left: word.x, top: word.y, width: word.w, height: word.h, borderRadius: 14, borderWidth: 2, borderColor, backgroundColor: bgColor, alignItems: "center", justifyContent: "center", zIndex: 60 }}>
      <Animated.View style={{ opacity: op, transform: [{ scale: sc }], alignItems: "center", justifyContent: "center" }}>
        <Text style={{ fontSize: WORD_FONT, fontWeight: "900", color: word.isTarget ? C.green : C.red, letterSpacing: 0.3 }}>{word.text}</Text>
      </Animated.View>
    </Animated.View>
  );
}
...
function SpeedBar({ speed }) {
  ...
  return (
    <View style={{ position: "absolute", bottom: 50, left: 24, right: 24, height: 8, borderRadius: 4, backgroundColor: "rgba(255,255,255,0.1)", borderWidth: 1, borderColor: "rgba(255,255,255,0.15)", zIndex: 80 }} pointerEvents="none">
      <View style={{ width: `${Math.round(frac * 100)}%`, height: "100%", borderRadius: 4, backgroundColor: color, shadowColor: color, shadowOffset: { width: 0, height: 0 }, shadowOpacity: 0.8, shadowRadius: 6 }} />
    </View>
  );
}
...
function CatchPopup({ sw, sh, catches, onDone }) {
  ...
  useEffect(() => {
    Animated.parallel([
      Animated.spring(sc, { toValue: 1, friction: 3, tension: 100, useNativeDriver: true }),
      Animated.timing(op, { toValue: 1, duration: 200, useNativeDriver: true }),
    ]).start();
    Animated.loop(
      Animated.sequence([
        Animated.timing(shimX, { toValue: 240, duration: 900, easing: Easing.inOut(Easing.quad), useNativeDriver: true }),
        Animated.timing(shimX, { toValue: -160, duration: 0, useNativeDriver: true }),
      ]),
    ).start();
    const t = setTimeout(() => Animated.timing(op, { toValue: 0, duration: 350, useNativeDriver: true }).start(onDone), 1400);
    return () => clearTimeout(t);
  }, []);
  return (
    <Animated.View pointerEvents="none" style={{ position: "absolute", left: sw * 0.08, right: sw * 0.08, top: sh * 0.3, zIndex: 300, opacity: op, transform: [{ scale: sc }] }}>
      <View style={{ backgroundColor: "rgba(10,10,10,0.96)", borderRadius: 22, borderWidth: 2.5, borderColor: C.yellow, overflow: "hidden", paddingVertical: 18, paddingHorizontal: 24, alignItems: "center", shadowColor: C.yellow, shadowOffset: { width: 0, height: 0 }, shadowOpacity: 0.9, shadowRadius: 24, elevation: 16 }}>
        <Animated.View pointerEvents="none" style={{ position: "absolute", top: 0, bottom: 0, width: 80, backgroundColor: "rgba(255,255,255,0.12)", transform: [{ translateX: shimX }, { skewX: "-15deg" }] }} />
        <Text style={{ fontSize: 36, marginBottom: 4 }}>🚨</Text>
        <Text style={{ fontSize: 22, fontWeight: "900", color: C.yellow, letterSpacing: 1, textAlign: "center" }}>GREAT CATCH!</Text>
        <Text style={{ fontSize: 32, fontWeight: "900", color: C.white, marginTop: 4 }}>+{PTS_CATCH}</Text>
        <Text style={{ fontSize: 13, color: C.textSec, marginTop: 2 }}>{catches} {catches === 1 ? "car" : "cars"} caught!</Text>
      </View>
    </Animated.View>
  );
}
...
function Notif({ text, color, icon, sw, sh, onDone }) {
  ...
  useEffect(() => {
    ...
    const t = setTimeout(() => Animated.timing(op, { toValue: 0, duration: 260, useNativeDriver: true }).start(onDone), 1200);
    return () => clearTimeout(t);
  }, []);
  return (
    <Animated.View pointerEvents="none" style={{ position: "absolute", left: sw * 0.1, right: sw * 0.1, top: sh * 0.55, zIndex: 500, opacity: op, transform: [{ scale: sc }] }}>
      <View style={{ backgroundColor: "rgba(8,8,26,0.96)", borderRadius: 18, borderWidth: 2, borderColor: color, paddingVertical: 11, paddingHorizontal: 16, alignItems: "center", shadowColor: color, shadowOffset: { width: 0, height: 0 }, shadowOpacity: 0.8, shadowRadius: 14, elevation: 12 }}>
        <Text style={{ fontSize: 20, marginBottom: 2 }}>{icon}</Text>
        <Text style={{ fontSize: 13, fontWeight: "900", color, textAlign: "center" }}>{text}</Text>
      </View>
    </Animated.View>
  );
}
...
function IdleOverlay({ onStart, onExit }) {
  ...
  return (
    <ImageBackground source={require("../../assets/games/police-patrol-chase/cover.jpg")} resizeMode="stretch" style={StyleSheet.absoluteFill}>
      <View style={{ alignItems: "center", justifyContent: "center", marginTop: "140%" }}>
        <Animated.View style={{ transform: [{ scale: pulse }] }}>
          <TouchableOpacity style={st.blueBtn} onPress={onStart} activeOpacity={0.85}>
    ...
    <View style={st.overlayBg}>
      <Animated.View style={[st.card, { transform: [{ translateY: slide }], opacity: fade }]}>
        <View style={st.roundHeader}>
          ...
          <Text style={st.roundHeaderTxt}>🚨 DISPATCH ROUND {round.id}/{totalRounds}</Text>
        </View>
        ...
        <Text style={{ color: C.textSec, fontSize: font.md, fontWeight: "700", marginBottom: pad.sm, letterSpacing: 1 }}>SPEED BOOST WORDS:</Text>
        <View style={st.chipRow}>
          {round.targetWords.slice(0, 8).map((w) => (
            <View key={w} style={st.chip}><Text style={st.chipText}>{w}</Text></View>
          ))}
        </View>
        <TouchableOpacity style={[st.blueBtn, { width: "100%" }]} onPress={onStart} activeOpacity={0.85}>
...
function ResultOverlay({ score, catches, isLast, onNext, onExit }) {
  ...
  const title = stars === 3 ? "Top Officer! 🏆" : stars === 2 ? "Good Chase! ⭐" : "Keep Trying!";
  useEffect(() => {
    Animated.parallel([
    ...
    <View style={st.overlayBg}>
      <Animated.View style={[st.resultCard, { opacity: op, transform: [{ scale: sc }] }]}>
        ...
        <View style={st.statRow}>
          <View style={st.statPill}><Text style={st.statLabel}>CAUGHT</Text><Text style={[st.statVal, { color: C.green }]}>{catches}</Text></View>
...
function FinalOverlay({ totalScore, onRestart, onExit }) {
  const grade = totalScore >= 400 ? "Chief Officer! 🏆" : totalScore >= 200 ? "Detective! 🌟" : "Cadet 🚨";
  const sc = useRef(new Animated.Value(0.5)).current;
  const op = useRef(new Animated.Value(0)).current;
  const pulse = useRef(new Animated.Value(1)).current;
  const shimX = useRef(new Animated.Value(-220)).current;
...
export default function PolicePursuitGame({ onExit }) {
  ...
  const rounds = useMemo(() => {
    try {
      ...
      if (
        ...
        console.log("[PolicePursuitGame] using backend rounds, count:", parsed.length);
        return parsed;
      }
      console.log("[PolicePursuitGame] invalid shape — using fallback");
  ...
  const policeAnimX = useRef(new Animated.Value(LANE_CENTRES[POLICE_START_LANE] - POLICE_W / 2)).current;
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      const assets = [
        [sndIntro, require("../../assets/sounds/game/police-pursuit/intro.mp3")],
        [sndDoor, require("../../assets/sounds/game/police-pursuit/door-open.mp3")],
        [sndSiren, require("../../assets/sounds/game/police-pursuit/siren.mp3")],
        [sndCorrect, require("../../assets/sounds/game/police-pursuit/correct-hit.mp3")],
        [sndWrong, require("../../assets/sounds/game/police-pursuit/wrong-hit.mp3")],
        [sndCatch, require("../../assets/sounds/game/police-pursuit/caught.mp3")],
      ...
      if (alive && phaseRef.current === "idle") {
        try { sndIntro.current?.playAsync(); } catch (_) {}
      }
    })();
    return () => {
      alive = false;
      [sndIntro, sndDoor, sndSiren, sndCorrect, sndWrong, sndCatch, sndWin, sndLose].forEach((r) => {
        r.current?.unloadAsync(); r.current = null;
      });
    };
  }, []);

  // ── Phase-driven audio — reads from resolved `rounds` for TTS ────────────
  useEffect(() => {
    if (phase === "idle") {
      try { sndIntro.current?.setPositionAsync(0).then(() => sndIntro.current?.playAsync()); } catch (_) {}
    } else {
      try { sndIntro.current?.stopAsync(); } catch (_) {}
    }

    if (phase === "briefing") {
      // Reads from resolved rounds — not FALLBACK_ROUNDS
    ...
    if (phase !== "briefing") { Speech.stop(); }
  }, [phase, roundIndex, rounds]); // `rounds` in deps so TTS always uses current data
  ...
  const moveLane = useCallback((dir) => {
    ...
    const next = Math.max(0, Math.min(NUM_LANES - 1, policeLaneRef.current + dir));
    ...
    Animated.spring(policeAnimX, { toValue: targetX, friction: 8, tension: 140, useNativeDriver: false }).start();
  }, [LANE_CENTRES, policeAnimX]);

  const spawnStars = (x, y) => {
    const s = Array.from({ length: 6 }, (_, i) => ({
      id: starIdRef.current++, startX: x - 13, startY: y - 13, endX: BADGE_X, endY: BADGE_Y, delay: i * 65,
    }));
  ...
  const startSiren = () => {
    ...
    sirenLoop.current = Animated.loop(
      Animated.sequence([
        Animated.timing(sirenAnim, { toValue: 1, duration: 300, useNativeDriver: false }),
        Animated.timing(sirenAnim, { toValue: 0, duration: 300, useNativeDriver: false }),
      ]),
    );
  ...
  const stopSiren = () => {
    ...
    try { sndSiren.current?.stopAsync(); } catch (_) {}
  };
  ...
  // ── Game tick — tickFnRef pattern (assigned each render, called by interval) ─
  tickFnRef.current = () => {
    if (phaseRef.current !== "playing") return;
    tickRef.current++;

    if (speedRef.current > SPEED_NORMAL) speedRef.current = Math.max(SPEED_NORMAL, speedRef.current - SPEED_DECAY);
    else if (speedRef.current < SPEED_NORMAL) speedRef.current = Math.min(SPEED_NORMAL, speedRef.current + SPEED_DECAY);

    dashRef.current = (dashRef.current + speedRef.current) % 460;

    const delta = speedRef.current - SPEED_NORMAL;
    if (delta > 0) blackYRef.current = Math.min(POLICE_Y - CATCH_DIST - BLACK_H, blackYRef.current + delta * CLOSE_RATE);
    else if (delta < 0) blackYRef.current = Math.max(sh * 0.05, blackYRef.current + delta * OPEN_RATE);
    ...
    if (tickRef.current % SPAWN_TICKS === 0) {
      if (queueIdxRef.current >= queueRef.current.length) queueIdxRef.current = 0;
    ...
    let correctHit = null, wrongHit = null;

    wordsRef.current = wordsRef.current
      .map((w) => {
        if (w.hit) return w;
        const ny = w.y + WORD_SPEED;
        if (w.x < pRight && w.x + w.w > pLeft && ny + w.h >= pTop && ny <= pBot) {
      ...
      .filter((w) => w.y < sh + 20 && !(w.hit && w.hitTick && tickRef.current - w.hitTick > 20));

    if (correctHit) {
      speedRef.current = SPEED_BOOST;
      scoreRef.current += PTS_CORRECT;
      playSound(sndCorrect);
      setNotif({ id: uid(), text: `🚀 Speed boost! +${PTS_CORRECT}`, color: C.green, icon: "💨" });
    }
    if (wrongHit) {
      speedRef.current = SPEED_SLOW;
      scoreRef.current += PTS_WRONG;
      playSound(sndWrong);
      setNotif({ id: uid(), text: `🐌 Slowing down! ${PTS_WRONG}`, color: C.red, icon: "⚠️" });
    }
  ...
  const startRound = useCallback(
    (idx) => {
      ...
      setScore(0); setCatches(0); setWords([]); setStars([]);
      setNotif(null); setCatchPopup(null);
      setBlackCarY(sh * START_GAP); setSpeed(SPEED_NORMAL); setTimeLeft(ROUND_SECS);
    ...
    [sh, endRound, LANE_CENTRES, policeAnimX, rounds], // `rounds` in deps — always fresh
  );
  ...
  const handleStartRound = () => { playSound(sndDoor); startRound(roundIndex); };
  ...
  return (
    <View style={st.root}>
      {phase !== "idle" && (
        <View style={{ position: "absolute", top: 0, left: 0, right: 0, height: HEADER_H, backgroundColor: C.header, zIndex: 200, borderBottomWidth: 2, borderBottomColor: "rgba(0,188,212,0.4)", flexDirection: "row", alignItems: "flex-end", paddingBottom: 10, paddingHorizontal: 14 }}>
          <TouchableOpacity style={st.headerExitBtn} onPress={handleExit} activeOpacity={0.8} hitSlop={{ top: 10, bottom: 10, left: 10, right: 10 }}>
            <Text style={st.exitTxt}>✕</Text>
          </TouchableOpacity>
          <View style={{ flex: 1, alignItems: "center" }}>
            {isPlaying && (
              <>
                <Text style={{ fontSize: 12, fontWeight: "800", color: C.teal, letterSpacing: 0.5 }}>{round.emoji} {round.category}</Text>
                <Text style={{ fontSize: 20, fontWeight: "900", color: C.white, ...(timeLeft <= 10 && { color: C.red }) }}>{timeLeft}s</Text>
              </>
            )}
            {!isPlaying && <Text style={{ fontSize: 16, fontWeight: "900", color: C.teal, letterSpacing: 1 }}>🚨 POLICE PURSUIT</Text>}
          </View>
      ...
      {phase !== "idle" && <Road sw={sw} sh={sh} dashOffset={dashOffset} />}

      {phase !== "idle" && (
        <View style={[StyleSheet.absoluteFill, { top: HEADER_H, zIndex: 1 }]} pointerEvents="none">
          <View style={{ position: "absolute", top: 0, left: 0, right: 0, height: sh * 0.18, backgroundColor: "rgba(13,13,40,0.8)" }} />
          {Array.from({ length: 18 }, (_, i) => (
            <View key={i} style={{ position: "absolute", left: (i * 113 + 17) % sw, top: (i * 37 + 5) % (sh * 0.16), width: 2, height: 2, borderRadius: 1, backgroundColor: "rgba(255,255,255,0.6)", opacity: 0.5 }} />
          ))}
        </View>
      )}
      ...
      {isPlaying && <BlackCar x={LANE_CENTRES[1] - BLACK_W / 2} y={blackCarY} />}

      {(isPlaying || phase === "briefing") && (
        <Animated.View pointerEvents="none" style={{ position: "absolute", left: policeAnimX, top: POLICE_Y, width: POLICE_W, height: POLICE_H, zIndex: 70 }}>
      ...
      {isPlaying && (
        <>
          <TouchableOpacity style={{ position: "absolute", left: 0, top: HEADER_H, width: sw / 2, bottom: 0, zIndex: 50 }} onPress={() => moveLane(-1)} activeOpacity={0.01}>
            <View style={{ position: "absolute", left: 16, top: "42%", opacity: 0.28 }}><Text style={{ fontSize: 36, color: C.white }}>◀</Text></View>
          </TouchableOpacity>
          <TouchableOpacity style={{ position: "absolute", right: 0, top: HEADER_H, width: sw / 2, bottom: 0, zIndex: 50 }} onPress={() => moveLane(1)} activeOpacity={0.01}>
            <View style={{ position: "absolute", right: 16, top: "42%", opacity: 0.28 }}><Text style={{ fontSize: 36, color: C.white }}>▶</Text></View>
          </TouchableOpacity>
        </>
      )}
      ...
      {isPlaying && (
        <View style={{ position: "absolute", bottom: 62, left: 0, right: 0, alignItems: "center", zIndex: 81 }} pointerEvents="none">
          <Text style={{ fontSize: 11, fontWeight: "700", color: speed > SPEED_NORMAL + 0.5 ? C.green : speed < SPEED_NORMAL - 0.5 ? C.red : C.textSec }}>
            {speed > SPEED_NORMAL + 0.5 ? "🚀 BOOSTING!" : speed < SPEED_NORMAL - 0.5 ? "🐌 SLOWING..." : "SPEED"}
          </Text>
        </View>
      )}
      ...
      {catchPopup && <CatchPopup key={catchPopup.id} sw={sw} sh={sh} catches={catchPopup.catches} onDone={() => setCatchPopup(null)} />}

      {notif && <Notif key={notif.id} text={notif.text} color={notif.color} icon={notif.icon} sw={sw} sh={sh} onDone={() => setNotif(null)} />}

      {isPlaying && (
        <View style={{ position: "absolute", bottom: 80, right: 16, backgroundColor: "rgba(0,0,0,0.7)", borderRadius: 14, borderWidth: 1.5, borderColor: C.tealBorder, paddingHorizontal: 10, paddingVertical: 5, zIndex: 100 }}>
          <Text style={{ fontSize: 12, fontWeight: "900", color: C.teal }}>🚗 {catches}</Text>
        </View>
      )}
      ...
      {phase === "briefing" && (
        // totalRounds passed as prop — RoundBriefing no longer touches FALLBACK_ROUNDS
      ...
      {phase === "result" && lastResult && (
        ...
        <ResultOverlay score={lastResult.score} catches={lastResult.catches} isLast={roundIndex + 1 >= totalRounds} onNext={handleNext} onExit={handleExit} />
      )}
...
const st = StyleSheet.create({
  ...
  headerExitBtn: { width: 36, height: 36, borderRadius: 18, backgroundColor: "rgba(255,255,255,0.08)", borderWidth: 1, borderColor: "rgba(255,255,255,0.15)", alignItems: "center", justifyContent: "center" },
  exitTxt: { fontSize: 13, color: C.textSec, fontWeight: "700" },
  scorePill: { backgroundColor: C.yellowDim, borderRadius: 18, borderWidth: 1.5, borderColor: C.yellowBorder, paddingHorizontal: 12, paddingVertical: 5, minWidth: 70, alignItems: "center" },
  scoreTxt: { fontSize: 14, fontWeight: "900", color: C.yellow },
  overlayBg: { ...StyleSheet.absoluteFillObject, backgroundColor: "rgba(8,8,26,0.92)", alignItems: "center", justifyContent: "center", zIndex: 400, paddingHorizontal: 22 },
  card: { width: "100%", maxWidth: 420, backgroundColor: "rgba(8,8,26,0.98)", borderRadius: 28, borderWidth: 1.5, borderColor: C.tealBorder, padding: 24, alignItems: "center", shadowColor: C.teal, shadowOffset: { width: 0, height: 0 }, shadowOpacity: 0.28, shadowRadius: 24, elevation: 10 },
  roundHeader: { width: "100%", backgroundColor: C.tealDim, borderRadius: 10, borderWidth: 1, borderColor: C.tealBorder, paddingVertical: 10, alignItems: "center", marginBottom: 12 },
  roundHeaderTxt: { fontSize: font.lg, fontWeight: "900", color: C.teal, letterSpacing: 1.5 },
  cardTitle: { fontSize: font.xxl, fontWeight: "900", color: C.white, marginBottom: 8, textAlign: "center" },
  cardDesc: { fontSize: font.md, color: C.textSec, textAlign: "center", lineHeight: 20, marginBottom: pad.xl },
  chipRow: { flexDirection: "row", flexWrap: "wrap", gap: 10, justifyContent: "center", marginBottom: pad.xxl },
  chip: { backgroundColor: C.greenDim, borderColor: C.greenBorder, borderWidth: 1.5, borderRadius: 14, paddingHorizontal: 10, paddingVertical: 3 },
  chipText: { color: C.green, fontWeight: "800", fontSize: font.lg },
  blueBtn: { backgroundColor: C.police, borderRadius: 30, paddingHorizontal: 36, paddingVertical: 15, alignItems: "center", shadowColor: C.teal, shadowOffset: { width: 0, height: 0 }, shadowOpacity: 0.7, shadowRadius: 16, elevation: 10, borderWidth: 2, borderColor: C.teal },
  blueBtnText: { fontSize: font.h3, fontWeight: "900", color: C.white, letterSpacing: 0.5 },
  resultCard: { width: "100%", maxWidth: 400, backgroundColor: "rgba(8,8,26,0.98)", borderRadius: 28, borderWidth: 1.5, borderColor: "rgba(255,213,79,0.45)", padding: 24, alignItems: "center", shadowColor: C.yellow, shadowOffset: { width: 0, height: 0 }, shadowOpacity: 0.28, shadowRadius: 24, elevation: 10 },
  ...
  goldBtnText: { fontSize: 16, fontWeight: "900", color: C.bg, letterSpacing: 0.4 },
