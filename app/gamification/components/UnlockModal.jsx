...
function FlyingDiamond({ fromX, fromY, toX, toY, delay, onLand }) {
  ...
  const translateY = progress.interpolate({
    inputRange: [0, 0.4, 1], outputRange: [fromY - 14, peakY, toY - 14], extrapolate: "clamp",
  });
  ...
  return (
    <Animated.View style={{
      ...
      opacity: op, zIndex: 400, pointerEvents: "none",
...
function MiniGameCard({ slot, size: boxSize = ICON_BOX }) {
  ...
  return (
    <View style={{ width: boxSize, height: boxSize, borderRadius: radius.lg, overflow: "hidden" }}>
      ...
      <View style={{ ...StyleSheet.absoluteFillObject, alignItems: "center", justifyContent: "center", transform: [{ scale: 0.55 }] }}>
        {MiniAnim ? <MiniAnim /> : <Text style={{ fontSize: 32 }}>{slot?.icon ?? "🎮"}</Text>}
      </View>
    </View>
  );
}
...
export default function UnlockModal() {
  ...
  useEffect(() => {
    if (!visible) {
      ...
      Animated.parallel([
        Animated.timing(sheetY, { toValue: SH, duration: 350, easing: Easing.in(Easing.quad), useNativeDriver: true }),
        Animated.timing(scrOp,  { toValue: 0, duration: 280, useNativeDriver: true }),
      ]).start(() => reset());
      return;
    }
  ...
  const handleCancel = useCallback(() => {
    walletPulseLoop.current?.stop();
    Animated.parallel([
      Animated.timing(sheetY, { toValue: SH, duration: 380, easing: Easing.in(Easing.quad), useNativeDriver: true }),
  ...
  return (
    <Modal transparent visible={visible} animationType="none" onRequestClose={handleCancel}>
      <View style={s.shell} pointerEvents="box-none">
        ...
        {flyingDiamonds.map((d) => <FlyingDiamond key={d.id} {...d} onLand={handleCoinLand} />)}
        ...
        >
          <View style={s.handle} />
          <Text style={s.heading}>{panel === "done" ? "🎉 Game Unlocked!" : "💎 Unlock This Game"}</Text>
            ...
            <Animated.View style={{ opacity: rowOp, width: "100%" }}>
              <View style={s.rewardRow}>
                <View style={s.iconCol}>
                  <Animated.View style={[s.iconBox, s.walletBox, { transform: [{ scale: Animated.multiply(walletScale, walletPulse) }] }]}>
                    <ExpoImage source={require("../../../assets/img/bag.png")} style={s.boxImage} contentFit="contain" cachePolicy="memory-disk" />
                    <View style={s.walletCounter}>
                      <ExpoImage source={require("../../../assets/img/diamond.png")} style={s.counterIcon} contentFit="contain" cachePolicy="memory-disk" />
                ...
                <View style={s.arrowTrail}>
                  {[0, 1, 2].map((i) => (
                    <Text key={i} style={[s.arrowChar, { opacity: panel === "animating" ? 1 : 0.3 }]}>›</Text>
                  ))}
                </View>

                <View style={s.iconCol}>
                  <Animated.View style={[s.iconBox, s.gameBox, { transform: [{ scale: gameScale }, { translateX: walletShakeX }] }]}>
              ...
              {panel === "confirm" && (
                <View style={s.btnRow}>
                  ...
                  {canAfford ? (
                    <TouchableOpacity style={s.confirmBtn} onPress={handleConfirmTap} activeOpacity={0.85}>
                      <Text style={s.confirmBtnText}>Unlock 💎 {COST}</Text>
                    </TouchableOpacity>
                  ) : (
                    <View style={s.cantAffordBox}>
                      <Text style={s.cantAffordText}>Need {COST - diamonds} more 💎</Text>
                    </View>
                  )}
                </View>
              )}
              ...
              {panel === "animating" && (
                <Text style={s.animatingHint}>✨ Transferring {COST} diamonds...</Text>
              )}
            </Animated.View>
          )}
...
const s = StyleSheet.create({
  ...
  scrim:  { position: "absolute", top: 0, left: 0, right: 0, height: TOP_CLEAR + 20, backgroundColor: "rgba(0,0,0,0.5)" },
  sheet:  {
    position: "absolute", bottom: 0, left: 0, right: 0,
    minHeight: SHEET_HEIGHT, backgroundColor: C.bg,
    ...
    paddingHorizontal: CARD_PADDING, paddingBottom: pad.xl,
    shadowColor: C.purple, shadowOffset: { width: 0, height: -8 },
    shadowOpacity: 0.4, shadowRadius: 20, elevation: 24,
  },
  handle:     { width: 44, height: 5, borderRadius: 3, backgroundColor: "rgba(255,255,255,0.2)", marginBottom: pad.sm },
  heading:    { fontFamily: FONTS.bold, fontSize: font.xl, color: C.textPri, textAlign: "center", letterSpacing: 0.3, marginBottom: pad.xs, marginTop: pad.sm, textShadowColor: C.purpleGlow, textShadowOffset: { width: 0, height: 0 }, textShadowRadius: 10 },
  ...
  confirmBtn:    { flex: 1.5, paddingVertical: pad.sm, borderRadius: radius.pill, borderWidth: 1.5, borderColor: C.purpleBorder, backgroundColor: "rgba(179,157,219,0.15)", alignItems: "center", shadowColor: C.purple, shadowOffset: { width: 0, height: 0 }, shadowOpacity: 0.5, shadowRadius: 8, elevation: 6 },
