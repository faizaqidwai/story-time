...
const START_GAP     = 0.30;  // black car starts at 30% of screen height
...
function PoliceCar({ x, y, sirenAnim }) {
  ...
  return (
    <View pointerEvents="none" style={{
      ...
      width:POLICE_W, height:POLICE_H,
      zIndex:70,
    }}>
      {/* Body */}
      <View style={{
        ...
        {/* Windshield (top) */}
        <View style={{position:"absolute",left:6,top:6,right:6,height:16,
          backgroundColor:"rgba(144,200,255,0.6)",borderRadius:4,borderWidth:1,borderColor:"rgba(255,255,255,0.3)"}}/>
        {/* Rear window (bottom) */}
        <View style={{position:"absolute",left:8,bottom:8,right:8,height:12,
          backgroundColor:"rgba(144,200,255,0.5)",borderRadius:3}}/>
        {/* Wheels — 4 corners */}
        {[{top:8,left:-5},{top:8,right:-5},{bottom:8,left:-5},{bottom:8,right:-5}].map((pos,i)=>(
          <View key={i} style={{
            position:"absolute", ...pos,
            width:8, height:14, borderRadius:3,
            backgroundColor:"#222", borderWidth:1, borderColor:"#555",
          }}/>
        ))}
        {/* POLICE text */}
        <View style={{position:"absolute",left:0,right:0,top:POLICE_H*0.44,alignItems:"center"}}>
          <Text style={{fontSize:7,fontWeight:"900",color:C.policeStripe,letterSpacing:0.5}}>POLICE</Text>
        </View>
      </View>
      ...
      <View style={{
        position:"absolute",
        left:POLICE_W*0.1, top:POLICE_H*0.16,
        width:POLICE_W*0.8, height:10,
        borderRadius:5, backgroundColor:"#111",
      }}/>
      {/* Red half */}
      <Animated.View style={{
        position:"absolute",
        left:POLICE_W*0.1, top:POLICE_H*0.16,
        width:POLICE_W*0.38, height:10,
        borderRadius:5,
        backgroundColor:C.siren1,
        opacity:redOp,
        shadowColor:C.siren1, shadowOffset:{width:0,height:0},
        shadowOpacity:1, shadowRadius:8, elevation:12,
      }}/>
      {/* Blue half */}
      <Animated.View style={{
        position:"absolute",
        left:POLICE_W*0.52, top:POLICE_H*0.16,
        width:POLICE_W*0.38, height:10,
        borderRadius:5,
        backgroundColor:C.siren2,
        opacity:blueOp,
        shadowColor:C.siren2, shadowOffset:{width:0,height:0},
        shadowOpacity:1, shadowRadius:8, elevation:12,
      }}/>
    </View>
  );
  ...
  return (
    <View pointerEvents="none" style={{
      ...
      <View style={{
        ...
        {/* Windshield */}
        <View style={{position:"absolute",left:5,top:5,right:5,height:14,
          backgroundColor:"rgba(100,180,255,0.4)",borderRadius:3}}/>
        {/* Rear */}
        <View style={{position:"absolute",left:7,bottom:7,right:7,height:11,
          backgroundColor:"rgba(100,180,255,0.3)",borderRadius:3}}/>
        {/* Red tail lights */}
        <View style={{position:"absolute",bottom:2,left:3,width:8,height:5,
          backgroundColor:"#FF1744",borderRadius:2,
          shadowColor:"#FF1744",shadowOffset:{width:0,height:0},shadowOpacity:1,shadowRadius:4}}/>
        <View style={{position:"absolute",bottom:2,right:3,width:8,height:5,
          backgroundColor:"#FF1744",borderRadius:2,
          shadowColor:"#FF1744",shadowOffset:{width:0,height:0},shadowOpacity:1,shadowRadius:4}}/>
        {/* Wheels */}
        {[{t:6,l:-5},{t:6,r:-5},{b:6,l:-5},{b:6,r:-5}].map((pos,i)=>(
          <View key={i} style={{
            position:"absolute", ...pos,
            width:7, height:13, borderRadius:3,
            backgroundColor:"#111", borderWidth:1, borderColor:"#444",
          }}/>
        ))}
      </View>
    </View>
  ...
  return (
    ...
    <Animated.View pointerEvents="none" style={{
      ...
      <Animated.View style={{
        ...
        <Text style={{
          fontSize:WORD_FONT, fontWeight:"900",
...
function SpeedBar({ speed }) {
  ...
  return (
    <View style={{
      ...
      <View style={{
        width:`${Math.round(frac*100)}%`, height:"100%",
...
function CatchPopup({ sw, sh, catches, onDone }) {
  ...
  useEffect(() => {
    ...
    const t = setTimeout(()=>{
      Animated.timing(op,{toValue:0,duration:350,useNativeDriver:true}).start(onDone);
    }, 1400);
...
function FlyingStar({ startX, startY, endX, endY, delay, onDone }) {
  ...
  return (
    <Animated.View pointerEvents="none" style={{
      position:"absolute",top:0,left:0,width:26,height:26,zIndex:999,
...
function Notif({ text, color, icon, sw, sh, onDone }) {
  ...
  useEffect(() => {
    ...
    const t = setTimeout(()=>
      Animated.timing(op,{toValue:0,duration:260,useNativeDriver:true}).start(onDone),1200);
    return ()=>clearTimeout(t);
  },[]);
  return (
    <Animated.View pointerEvents="none" style={{
      position:"absolute", left:sw*0.1, right:sw*0.1, top:sh*0.55,
      zIndex:500, opacity:op, transform:[{scale:sc}],
    }}>
      <View style={{
        backgroundColor:"rgba(8,8,26,0.96)",
        borderRadius:18, borderWidth:2, borderColor:color,
        paddingVertical:11, paddingHorizontal:16, alignItems:"center",
        shadowColor:color,shadowOffset:{width:0,height:0},shadowOpacity:0.8,shadowRadius:14,elevation:12,
      }}>
  ...
  return (
    <View style={st.overlayBg}>
      ...
      <Text style={st.idleHint}>
        ...
        Collect <Text style={{color:C.green,fontWeight:"700"}}>correct words</Text> to speed up!{"\n"}
    ...
    <View style={st.overlayBg}>
      <Animated.View style={[st.card,{transform:[{translateY:slide}],opacity:fade}]}>
        <View style={st.roundHeader}>
          <Text style={st.roundHeaderTxt}>🚨  DISPATCH  ROUND {round.id}/{ROUNDS.length}</Text>
        </View>
        ...
        <Text style={{color:C.textSec,fontSize:11,fontWeight:"700",marginBottom:8,letterSpacing:1}}>SPEED BOOST WORDS:</Text>
    ...
    <View style={st.overlayBg}>
      <Animated.View style={[st.resultCard,st.finalCard,{opacity:op,transform:[{scale:sc}]}]}>
        ...
        <View style={{flexDirection:"row",gap:10,marginBottom:10}}>
          {[s1,s2,s3].map((s,i)=>(<Animated.Text key={i} style={{fontSize:32,transform:[{scale:s}]}}>⭐</Animated.Text>))}
        </View>
...
export default function PolicePursuitGame({ onExit }) {
  ...
  useEffect(()=>{
    ...
    return()=>{ alive=false; [sndCorrect,sndWrong,sndWin,sndLose,sndCatch].forEach(r=>{r.current?.unloadAsync();r.current=null;}); };
  },[]);
  ...
  const startSiren = () => {
    ...
    sirenLoop.current = Animated.loop(Animated.sequence([
  ...
  const stopSiren = () => { sirenLoop.current?.stop(); sirenAnim.setValue(0); };
  ...
  tickFnRef.current = () => {
    ...
    if (delta > 0) {
      ...
      blackYRef.current = Math.min(POLICE_Y - CATCH_DIST - BLACK_H, blackYRef.current + delta * CLOSE_RATE);
    } else if (delta < 0) {
      // slowing — black car moves away (Y decreases toward top)
      blackYRef.current = Math.max(sh * 0.05, blackYRef.current + delta * OPEN_RATE);
    }
    ...
    if (blackYRef.current >= POLICE_Y - CATCH_DIST - BLACK_H) {
      ...
      playSound(sndCatch);
      const cx = policeXRef.current + POLICE_W/2, cy = POLICE_Y - 40;
      spawnStars(cx, cy);
  ...
  const startRound = useCallback((idx)=>{
    ...
    setScore(0); setCatches(0);
    setWords([]); setStars([]); setNotif(null); setCatchPopup(null);
  ...
  // ── Navigation ────────────────────────────────────────────
  const handleStartGame  = ()=>{ setRoundIndex(0); setTotalScore(0); phaseRef.current="briefing"; setPhase("briefing"); };
  ...
  return (
    <View style={st.root}>
      ...
      <View style={{
        ...
        height:HEADER_H, backgroundColor:C.header,
        zIndex:200,
        borderBottomWidth:2, borderBottomColor:"rgba(0,188,212,0.4)",
        flexDirection:"row", alignItems:"flex-end",
        paddingBottom:10, paddingHorizontal:14,
      }}>
        {/* Exit */}
        <TouchableOpacity style={st.headerExitBtn} onPress={handleExit} activeOpacity={0.8}
        ...
        <View style={{flex:1, alignItems:"center"}}>
          {isPlaying && (
            <>
              <Text style={{fontSize:12,fontWeight:"800",color:C.teal,letterSpacing:0.5}}>
          ...
          {!isPlaying && (
            <Text style={{fontSize:16,fontWeight:"900",color:C.teal,letterSpacing:1}}>
              🚨 POLICE PURSUIT
            </Text>
          )}
        </View>
      ...
      <View style={[StyleSheet.absoluteFill,{top:HEADER_H,zIndex:1}]} pointerEvents="none">
        {/* Sky */}
        <View style={{position:"absolute",top:0,left:0,right:0,height:sh*0.18,
          backgroundColor:"rgba(13,13,40,0.8)"}}/>
        {/* Stars */}
        {Array.from({length:18},(_,i)=>(
          <View key={i} style={{
            position:"absolute",
            left:(i*113+17)%sw, top:(i*37+5)%(sh*0.16),
            width:2,height:2,borderRadius:1,
            backgroundColor:"rgba(255,255,255,0.6)",opacity:0.5,
          }}/>
        ))}
      </View>
      ...
      {isPlaying && (
        <>
          <TouchableOpacity
            style={{position:"absolute",left:0,top:HEADER_H,width:sw/2,bottom:0,zIndex:50}}
            onPress={()=>moveLane(-1)}
            activeOpacity={0.01}
          >
            <View style={{position:"absolute",left:16,top:"42%",opacity:0.28}}>
              <Text style={{fontSize:36,color:C.white}}>◀</Text>
            </View>
          </TouchableOpacity>
          <TouchableOpacity
            style={{position:"absolute",right:0,top:HEADER_H,width:sw/2,bottom:0,zIndex:50}}
            onPress={()=>moveLane(1)}
            activeOpacity={0.01}
          >
            <View style={{position:"absolute",right:16,top:"42%",opacity:0.28}}>
              <Text style={{fontSize:36,color:C.white}}>▶</Text>
            </View>
          </TouchableOpacity>
        </>
      ...
      {isPlaying && (
        <View style={{
          ...
          alignItems:"center", zIndex:81,
        }} pointerEvents="none">
          <Text style={{
            fontSize:11, fontWeight:"700",
            color: speed > SPEED_NORMAL+0.5 ? C.green : speed < SPEED_NORMAL-0.5 ? C.red : C.textSec,
          }}>
      ...
      {/* ── Catch popup ── */}
      {catchPopup && (
        <CatchPopup key={catchPopup.id} sw={sw} sh={sh} catches={catchPopup.catches}
          onDone={()=>setCatchPopup(null)}/>
      )}

      {/* ── Notification ── */}
      {notif && (
        <Notif key={notif.id} text={notif.text} color={notif.color} icon={notif.icon}
          sw={sw} sh={sh} onDone={()=>setNotif(null)}/>
      )}
      ...
      {isPlaying && (
        <View style={{
          ...
          borderRadius:14, borderWidth:1.5, borderColor:C.tealBorder,
      ...
      {phase==="briefing" && <RoundBriefing round={round} onStart={handleStartRound} onExit={handleExit}/>}
      {phase==="result" && lastResult && (
        <ResultOverlay score={lastResult.score} catches={lastResult.catches}
          isLast={roundIndex+1>=ROUNDS.length} onNext={handleNext} onExit={handleExit}/>
      )}
      {phase==="final" && (
...
const st = StyleSheet.create({
  ...
  scorePill: {
    backgroundColor:C.yellowDim, borderRadius:18,
    borderWidth:1.5, borderColor:C.yellowBorder,
    paddingHorizontal:12, paddingVertical:5, minWidth:70, alignItems:"center",
  },
  ...
  overlayBg: {
    ...
    alignItems:"center", justifyContent:"center",
    zIndex:400, paddingHorizontal:22,
  },
  idleTitle: {
    fontSize:38, fontWeight:"900", color:C.teal,
    letterSpacing:0.5, textAlign:"center",
    textShadowColor:"rgba(0,188,212,0.6)",
    textShadowOffset:{width:0,height:0}, textShadowRadius:18,
    marginBottom:6,
  },
  idleSubtitle: { fontSize:14, fontWeight:"700", color:C.yellow, letterSpacing:2, marginBottom:18, textAlign:"center" },
  idleHint:     { fontSize:14, color:C.textSec, textAlign:"center", lineHeight:22, marginBottom:28, paddingHorizontal:8 },
  ...
  card: {
    ...
    shadowColor:C.teal, shadowOffset:{width:0,height:0},
    shadowOpacity:0.28, shadowRadius:24, elevation:10,
  },
  roundHeader: {
    width:"100%", backgroundColor:C.tealDim,
    borderRadius:10, borderWidth:1, borderColor:C.tealBorder,
    paddingVertical:10, alignItems:"center", marginBottom:12,
  },
  roundHeaderTxt: { fontSize:12, fontWeight:"900", color:C.teal, letterSpacing:1.5 },
  cardTitle: { fontSize:22, fontWeight:"900", color:C.white, marginBottom:8, textAlign:"center" },
  cardDesc:  { fontSize:13, color:C.textSec, textAlign:"center", lineHeight:20, marginBottom:14 },
  ...
  blueBtn: {
    backgroundColor:C.police, borderRadius:30,
  ...
  blueBtnText: { fontSize:17, fontWeight:"900", color:C.white, letterSpacing:0.5 },
  ...
  resultBannerText: {
    fontSize:20, fontWeight:"900", color:C.yellow, letterSpacing:1,
    textShadowColor:"rgba(255,213,79,0.6)",
    textShadowOffset:{width:0,height:0}, textShadowRadius:10,
  },
