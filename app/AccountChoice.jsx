...
export default function AccountChoice() {
  ...
  return (
    <View style={styles.root}>
      ...
      {/* Back button */}
      <TouchableOpacity
        style={[styles.backBtn, { width: sz.backBtnSize, height: sz.backBtnSize, borderRadius: sz.backBtnSize / 2, top: Platform.OS === "ios" ? 54 : 20 }]}
      ...
      <View style={[styles.inner, { paddingBottom: Platform.OS === "ios" ? sz.innerPaddingBottom_ios : sz.innerPaddingBottom_android }]}>
        {/* Logo */}
        <Animated.View style={[styles.logoWrap, { marginBottom: sz.logoMarginBottom, opacity: logoOp, transform: [{ translateY: logoY }] }]}>
          <Text style={[styles.appName, { fontSize: sz.appNameFontSize }]}>Story Time</Text>
          <Text style={[styles.appTagline, { fontSize: sz.appTaglineFontSize, lineHeight: sz.appTaglineLineHeight }]}>
        ...
        <View style={styles.cardsWrap}>
          ...
          <Animated.View style={{ opacity: card1Op, transform: [{ translateY: card1Y }] }}>
            <TouchableOpacity
              style={[styles.primaryCard, { padding: sz.cardPadding, borderRadius: sz.cardBorderRadius }]}
            ...
            >
              ...
              />
              <View style={[styles.cardIconWrap, { width: sz.cardIconBoxSize, height: sz.cardIconBoxSize }]}>
                {fetchingToken
              ...
              <View style={styles.cardText}>
                ...
                <Text style={[styles.cardSub, { fontSize: font.md, lineHeight: sz.cardSubLineHeight, marginTop: 3 }]}>
                  Set up your child's profile and start the adventure
                </Text>
              </View>
              {!fetchingToken && (
                <Text style={[styles.cardArrow, { color: C.teal, fontSize: sz.cardArrowFontSize }]}>›</Text>
              )}
            </TouchableOpacity>
          </Animated.View>

          {/* Divider */}
          <Animated.View style={[styles.divider, { opacity: dividerOp, marginVertical: sz.dividerMarginV }]}>
          ...
          <Animated.View style={{ opacity: card2Op, transform: [{ translateY: card2Y }] }}>
            <TouchableOpacity
              style={[styles.secondaryCard, { padding: sz.cardPadding, borderRadius: sz.cardBorderRadius }]}
              onPress={() => {
                playSound(require("../assets/sounds/sparkle.mp3"));
                setTimeout(() => router.replace("/EmailLogin?from=accountChoice"), 1000);
              }}
              activeOpacity={0.88}
            >
              <View style={[styles.cardIconWrap, { width: sz.cardIconBoxSize, height: sz.cardIconBoxSize }]}>
              ...
              <View style={styles.cardText}>
                <Text style={[styles.cardTitle, { color: C.textSec, fontSize: font.lg + 1 }]}>Already Have an Account</Text>
                <Text style={[styles.cardSub, { fontSize: font.md, lineHeight: sz.cardSubLineHeight, marginTop: 3 }]}>
                  Sign in with your email address
                </Text>
              </View>
              <Text style={[styles.cardArrow, { color: C.textMuted, fontSize: sz.cardArrowFontSize }]}>›</Text>
            </TouchableOpacity>
          </Animated.View>
        </View>

        {/* Footer note — unchanged */}
        <Animated.Text style={[styles.footerNote, { opacity: card2Op, fontSize: font.md, lineHeight: sz.footerLineHeight, marginTop: pad.sm }]}>
...
const styles = StyleSheet.create({
  ...
  glowTL:  { position: "absolute", top: -60, left: -60, width: 240, height: 240, borderRadius: 120, backgroundColor: "rgba(0,188,212,0.07)" },
  glowBR:  { position: "absolute", bottom: -40, right: -40, width: 200, height: 200, borderRadius: 100, backgroundColor: "rgba(150,82,217,0.07)" },
  backBtn: { position: "absolute", left: 20, zIndex: 20, backgroundColor: "rgba(255,255,255,0.06)", borderWidth: 1, borderColor: "rgba(255,255,255,0.12)", alignItems: "center", justifyContent: "center" },
  backIcon: { color: "#E0F7FA", marginTop: -2 },
  inner:   { flex: 1, paddingHorizontal: 24, justifyContent: "center" },
  logoWrap: { alignItems: "center" },
  appName:  { fontFamily: Platform.OS === "ios" ? "Noteworthy-Bold" : "sans-serif-condensed", fontWeight: "900", color: C.teal, letterSpacing: 1, textShadowColor: "rgba(0,188,212,0.4)", textShadowOffset: { width: 0, height: 0 }, textShadowRadius: 16, marginBottom: 8 },
  appTagline: { fontFamily: FONTS.light, color: C.textMuted, textAlign: "center", paddingHorizontal: 20 },
  cardsWrap: { gap: 0 },
  primaryCard:   { flexDirection: "row", alignItems: "center", gap: 14, borderWidth: 1.5, borderColor: C.tealBorder, overflow: "hidden", marginBottom: 0 },
  secondaryCard: { flexDirection: "row", alignItems: "center", gap: 14, borderWidth: 1.5, borderColor: "rgba(255,255,255,0.1)", backgroundColor: "rgba(255,255,255,0.04)" },
  cardIconWrap:  { borderRadius: 14, backgroundColor: "rgba(255,255,255,0.07)", alignItems: "center", justifyContent: "center", flexShrink: 0 },
  ...
  dividerText: { fontFamily: FONTS.regular, color: C.textMuted, letterSpacing: 0.5 },
  footerNote:  { fontFamily: FONTS.light, color: "rgba(255,255,255,0.2)", textAlign: "center", paddingHorizontal: 20 },
});
