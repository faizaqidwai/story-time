  ...
  return (
    <Animated.View pointerEvents="none" style={{
      ...
      <View style={{
        ...
        borderWidth:3, borderColor:C.eggBorder,
...
function CatchBurst({ x, y, isCorrect, onDone }) {
  ...
  return (
    <Animated.View pointerEvents="none" style={{
      ...
      width:72, height:72,
      alignItems:"center", justifyContent:"center",
      zIndex:200, opacity:op, transform:[{scale:sc}],
    }}>
...
function Notif({ text, color, icon, sw, sh, onDone }) {
  ...
  useEffect(() => {
    ...
    const t = setTimeout(()=>
      Animated.timing(op,{toValue:0,duration:280,useNativeDriver:true}).start(onDone), 1500);
    return ()=>clearTimeout(t);
  }, []);
  return (
    <Animated.View pointerEvents="none" style={{
      position:"absolute", left:sw*0.08, right:sw*0.08, top:sh*0.43,
      zIndex:500, opacity:op, transform:[{scale:sc}],
    }}>
      <View style={{
        backgroundColor:"rgba(5,20,5,0.97)",
        borderRadius:20, borderWidth:2.5, borderColor:color,
        paddingVertical:14, paddingHorizontal:20, alignItems:"center",
        shadowColor:color, shadowOffset:{width:0,height:0},
        shadowOpacity:0.9, shadowRadius:20, elevation:14,
      }}>
...
function TapZones({ sw, sh, onLeft, onRight, active }) {
  ...
  return (
    <>
      <TouchableOpacity
        style={{position:"absolute",left:0,top:0,width:sw/2,height:sh,zIndex:40}}
        onPress={onLeft} activeOpacity={1}>
        <View style={{position:"absolute",left:14,top:"50%",marginTop:-28,opacity:0.3}}>
          <Text style={{fontSize:38,color:"#FFF"}}>◀</Text>
        </View>
      </TouchableOpacity>
      <TouchableOpacity
        style={{position:"absolute",right:0,top:0,width:sw/2,height:sh,zIndex:40}}
        onPress={onRight} activeOpacity={1}>
  ...
  return (
    <View style={st.overlayBg}>
      ...
      <Text style={st.idleHint}>
        ...
        Tap <Text style={{color:"#69F0AE",fontWeight:"700"}}>◀ LEFT</Text> or <Text style={{color:"#69F0AE",fontWeight:"700"}}>RIGHT ▶</Text> to steer.{"\n"}
        Catch eggs whose words match the category!
      </Text>
      <Animated.View style={{transform:[{scale:pulse}]}}>
        <TouchableOpacity style={st.greenBtn} onPress={onStart} activeOpacity={0.85}>
    ...
    <View style={st.overlayBg}>
      ...
      <Animated.View style={[st.card,{transform:[{translateY:slide}],opacity:fade}]}>
        <View style={st.roundHeader}>
          <Text style={st.roundHeaderTxt}>🥚  ROUND {round.id} of {ROUNDS.length}</Text>
        </View>
...
export default function DinoWorldGame({ onExit }) {
  ...
  const startRound = useCallback((idx) => {
    ...
    setEggs([]); setStars([]); setBursts([]); setNotif(null);
    setDinoX(sw * 0.15); setDinoDir(1);
  ...
  return (
    <View style={st.root}>
      ...
      {/* ── Catch bursts ── */}
      {bursts.map(b=>(
        <CatchBurst key={b.id} x={b.x} y={b.y} isCorrect={b.isCorrect}
          onDone={()=>setBursts(prev=>prev.filter(x=>x.id!==b.id))}/>
      ))}

      {/* ── Flying stars ── */}
      {stars.map(s=>(
        <FlyingStar key={s.id} startX={s.startX} startY={s.startY}
          endX={s.endX} endY={s.endY} delay={s.delay}
      ...
      {phase==="result" && lastResult && (
        <ResultOverlay score={lastResult.score} caught={lastResult.caught} missed={lastResult.missed}
...
const st = StyleSheet.create({
  ...
  idleTitle: {
    fontSize:40, fontWeight:"900", color:"#69F0AE",
  ...
  idleSubtitle: {
    fontSize:14, fontWeight:"700", color:C.yellow,
    letterSpacing:2, marginBottom:20, textAlign:"center",
  },
  idleHint: {
    fontSize:14, color:"#B2DFDB",
  ...
  card: {
    ...
    borderRadius:28, borderWidth:1.5, borderColor:"rgba(105,240,174,0.4)",
    padding:24, alignItems:"center",
    shadowColor:"#69F0AE", shadowOffset:{width:0,height:0},
    shadowOpacity:0.25, shadowRadius:24, elevation:10,
  },
  roundHeader: {
    width:"100%", backgroundColor:"rgba(105,240,174,0.12)",
    borderRadius:10, borderWidth:1, borderColor:"rgba(105,240,174,0.35)",
  ...
  greenBtn: {
    ...
    borderWidth:2, borderColor:"#69F0AE",
  },
