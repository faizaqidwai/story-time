// app/features/onboarding/TermsAndPrivacy.jsx
//
...
//   ✅ C.bg "#08081a" → COLORS.background
//   ✅ C.teal "#00BCD4" → COLORS.primary (#00C4CC)
//   ✅ C.purple "#9652D9" → COLORS.purple (#7B2FBE)
//   ✅ C.textPri "#E0F7FA" → COLORS.textPrimary
//   ✅ C.textSec "#B0BEC5" → COLORS.textMuted (closest semantic match)
//   ✅ C.textMuted "#546E7A" → COLORS.textMuted
//   ✅ C.tealDim "rgba(0,188,212,0.12)" → COLORS.surfaceDim
//   ✅ C.tealBorder "rgba(0,188,212,0.3)" → COLORS.borderPrimary
//   ✅ Glow blobs → COLORS.glowCyan / COLORS.glowPurple
//   ✅ Title text shadow → COLORS.primary
//   ✅ C.surface, C.surfaceHigh, C.yellowDim unchanged (acceptable rgba values)
//   ✅ COLORS imported from theme
//
// UNTOUCHED (zero changes):
//   ✅ All accordion animation logic
//   ✅ All tab switching logic
//   ✅ All layout, sizing, padding
//   ✅ All legal content data (TERMS_SECTIONS, PRIVACY_SECTIONS)
//   ✅ All navigation
//   ✅ font.* and pad.* tokens
...
// ── Acceptable rgba constants — no theme token needed ────────────────────────
const SURFACE      = "rgba(255,255,255,0.04)";
const SURFACE_HIGH = "rgba(255,255,255,0.08)";
const YELLOW_DIM   = "rgba(255,213,79,0.10)";   // amber-tinted info box
...
const tabS = StyleSheet.create({
  pill: {
    flex: 1, paddingVertical: pad.sm,
    alignItems: "center", borderRadius: radius.pill,
    borderWidth: 1, borderColor: "transparent",
  },
  pillActive: {
    backgroundColor: COLORS.surfaceDim,         // ✅ was C.tealDim "rgba(0,188,212,0.12)"
    borderColor: COLORS.borderPrimary,           // ✅ was C.tealBorder "rgba(0,188,212,0.3)"
  },
  label:       { fontFamily: FONTS.bold, fontSize: font.md, color: COLORS.textMuted, letterSpacing: 0.3 }, // ✅
  labelActive: { color: COLORS.primary },        // ✅ was C.teal "#00BCD4"
});

// ─────────────────────────────────────────────────────────────────────────────
// SECTION CARD — layout/animation UNCHANGED, colours corrected
// ─────────────────────────────────────────────────────────────────────────────
function SectionCard({ title, content, index }) {
  ...
  const toggle = () => {
    ...
    Animated.parallel([
      ...
      Animated.timing(rotateAnim, { toValue, duration: 280, useNativeDriver: true }),
    ]).start();
  ...
  return (
    <View style={secS.card}>
      <TouchableOpacity style={secS.header} onPress={toggle} activeOpacity={0.75}>
...
const secS = StyleSheet.create({
  ...
  header: {
    ...
    paddingHorizontal: pad.md, paddingVertical: pad.sm, gap: pad.sm,
  },
  headerLeft: { flexDirection: "row", alignItems: "center", gap: pad.sm, flex: 1 },
  indexBadge: {
    width: 26, height: 26, borderRadius: 13,
    backgroundColor: COLORS.surfaceDim,         // ✅ was C.tealDim
    borderWidth: 1,
    borderColor: COLORS.borderPrimary,           // ✅ was C.tealBorder
    alignItems: "center", justifyContent: "center", flexShrink: 0,
  },
  indexText: { fontFamily: FONTS.bold, fontSize: font.xs, color: COLORS.primary },   // ✅
  title: { fontFamily: FONTS.bold, fontSize: font.md, color: COLORS.textPrimary, flex: 1, letterSpacing: 0.2 }, // ✅
  divider: { height: 1, backgroundColor: DIVIDER, marginBottom: pad.md },
  body: { paddingHorizontal: pad.md, paddingBottom: pad.md, position: "absolute", width: "100%" },
  content: { fontFamily: FONTS.regular, fontSize: font.sm, color: COLORS.textMuted, lineHeight: font.sm * 1.7, letterSpacing: 0.1 }, // ✅
});

// ─────────────────────────────────────────────────────────────────────────────
// CONTACT BANNER — layout UNCHANGED, colours corrected
// ─────────────────────────────────────────────────────────────────────────────
...
const cb = StyleSheet.create({
  container: {
    flexDirection: "row", alignItems: "center", gap: pad.md,
    backgroundColor: COLORS.surfaceDim,          // ✅ was C.tealDim
    borderRadius: radius.lg,
    borderWidth: 1,
    borderColor: COLORS.borderPrimary,            // ✅ was C.tealBorder
    padding: pad.md, marginBottom: pad.lg,
  },
  iconWrap: {
    width: 44, height: 44, borderRadius: 22,
    backgroundColor: "rgba(0,196,204,0.15)",      // cyan tinted — acceptable
    alignItems: "center", justifyContent: "center", flexShrink: 0,
  },
  icon:  { fontSize: font.xl },
  title: { fontFamily: FONTS.bold, fontSize: font.md, color: COLORS.textPrimary, marginBottom: 2 }, // ✅
  email: { fontFamily: FONTS.regular, fontSize: font.sm, color: COLORS.primary, letterSpacing: 0.2 }, // ✅
});

// ─────────────────────────────────────────────────────────────────────────────
// MAIN SCREEN — logic/navigation UNCHANGED, colours corrected
// ─────────────────────────────────────────────────────────────────────────────
  ...
  return (
    <View style={s.root}>
      ...
      <SafeAreaView style={{ flex: 1 }} edges={["top"]}>
        ...
        <View style={s.tabBar}>
          <TabPill label="Terms of Service" active={activeTab === "terms"}   onPress={() => switchTab("terms")} />
          <TabPill label="Privacy Policy"   active={activeTab === "privacy"} onPress={() => switchTab("privacy")} />
        </View>
        ...
        >
          <View style={s.heroBanner}>
            <Text style={s.heroEmoji}>{activeTab === "terms" ? "📋" : "🔐"}</Text>
          ...
          {sections.map((section, i) => (
            <SectionCard key={`${activeTab}-${i}`} title={section.title} content={section.content} index={i} />
          ))}
          ...
          <Text style={s.legalNote}>
            These documents were prepared for CodeKlusters / StoryTime. We recommend having a qualified legal professional review them, particularly regarding COPPA, GDPR, and jurisdiction-specific requirements.
          </Text>
...
// ── Styles — layout UNCHANGED, colours corrected ─────────────────────────────
const s = StyleSheet.create({
  root: { flex: 1, backgroundColor: COLORS.background },   // ✅ was C.bg "#08081a"
  glowTL: {
  ...
  header: {
    flexDirection: "row", alignItems: "center",
    paddingHorizontal: pad.md, paddingBottom: pad.sm,
    borderBottomWidth: 1,
    borderBottomColor: COLORS.surfaceDim,         // ✅ was "rgba(0,188,212,0.1)"
  },
  backBtn: {
  ...
  headerTitle: { fontFamily: FONTS.bold, fontSize: font.lg, color: COLORS.textPrimary, letterSpacing: 0.3 }, // ✅
  headerSub:   { fontFamily: FONTS.light, fontSize: font.xs, color: COLORS.textMuted, letterSpacing: 0.5, marginTop: 1 }, // ✅

  tabBar: {
    flexDirection: "row", gap: pad.s,
    marginHorizontal: pad.md, marginVertical: pad.sm,
    backgroundColor: SURFACE,
    borderRadius: radius.pill,
    padding: 4,
    borderWidth: 1, borderColor: DIVIDER,
  },
  scrollContent: { paddingHorizontal: pad.md, paddingTop: pad.sm },

  heroBanner: { alignItems: "center", paddingVertical: pad.lg, marginBottom: pad.sm },
  heroEmoji:  { fontSize: 48, marginBottom: pad.sm },
  heroTitle: {
    fontFamily: FONTS.bold, fontSize: font.xxl,
    color: COLORS.textPrimary,                    // ✅
    letterSpacing: 0.4, textAlign: "center",
    textShadowColor:  COLORS.primary,             // ✅ was hardcoded "rgba(0,188,212,0.4)"
    textShadowOffset: { width: 0, height: 0 },
    textShadowRadius: 12,
  },
  heroSub: { fontFamily: FONTS.light, fontSize: font.sm, color: COLORS.textMuted, marginTop: pad.xs, letterSpacing: 0.5 }, // ✅
  ...
  introBox: {
    ...
    padding: pad.md, marginBottom: pad.md,
  },
  introText: { fontFamily: FONTS.regular, fontSize: font.sm, color: COLORS.textMuted, lineHeight: font.sm * 1.6, textAlign: "center" }, // ✅

  legalNote: {
    fontFamily: FONTS.light, fontSize: font.xs,
    color: COLORS.textMuted,                      // ✅
    textAlign: "center",
    lineHeight: font.xs * 1.7,
    paddingHorizontal: pad.md, marginBottom: pad.md,
    fontStyle: "italic",
  },
});
