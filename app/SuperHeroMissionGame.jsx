...
const HERO_Y_FRAC  = 0.30;  // 30% from top
...
function makeCloud(sw, sh, startOffscreen=true) {
  ...
  return {
    ...
    y:    STATUS_H + 20 + Math.random() * (sh * 0.40),
    w, h,
...
function SuperHero({ x, y }) {
  ...
  return (
    <View pointerEvents="none" style={{
      ...
      <View style={{position:"absolute", left:10, top:18, gap:6, backgroundColor:"pink"}}>
      ...
      <View style={{
        ...
        alignItems:"center", justifyContent:"center",
        shadowColor: C.teal, shadowOffset:{width:2,height:0},
      ...
      <Animated.View style={{
        ...
        borderRadius: 8,
        backgroundColor:"#FFCCBC",
        borderWidth:2, borderColor:C.heroCape,
        shadowColor:C.red, shadowOffset:{width:3,height:0},
      ...
      <Animated.View pointerEvents="none" style={{
        position:"absolute",
        left:0, top:-5,
        width: HERO_W + 45, height: HERO_H + 12,
        borderRadius: 8,
...
function ComicFlash({ cx, topY, isCorrect, onDone }) {
  ...
  useEffect(() => {
    Animated.parallel([
      ...
      Animated.sequence([
        ...
        Animated.timing(op, {toValue:0, duration:FLASH_DURATION_MS*0.45, useNativeDriver:true}),
      ]),
    ]).start(onDone);
  }, []);
  ...
  const spikeAngles = Array.from({length:SPIKES}, (_,i) => (360/SPIKES)*i);
  ...
  return (
    ...
    >
      ...
      <View style={{
        ...
        borderWidth: 1.5, borderColor: C.buildingEdge,
        overflow: "hidden",
      }}>
        {/* Windows grid */}
        {Array.from({length: winRows}, (_,r) => (
          <View key={r} style={{
            ...
            paddingHorizontal:6, marginTop: r===0 ? WORD_PANEL_H+10 : 5,
          }}>
        ...
        <View style={{
          position:"absolute", top:-28, left:b.w/2-6,
          width:12, height:12, borderRadius:6,
          backgroundColor: C.red,
          shadowColor:C.red, shadowOffset:{width:0,height:0}, shadowOpacity:1, shadowRadius:8, elevation:4,
        }}/>
      </View>
...
function GroundStripe({ sw, sh, offsetX }) {
  return (
    <View pointerEvents="none" style={{
      ...
      borderTopWidth:2, borderTopColor: C.groundLine,
      zIndex:20,
    }}>
      {/* Dashes */}
      {Array.from({length:20},(_,i)=>(
        <View key={i} style={{
          ...
          top: 10, width:50, height:3,
          backgroundColor:"rgba(0,188,212,0.25)", borderRadius:2,
        }}/>
      ))}
    </View>
  );
...
function Notif({ text, color, icon, sw, sh, onDone }) {
  ...
  return (
    <Animated.View pointerEvents="none" style={{
      ...
      <View style={{
        backgroundColor:"rgba(6,8,24,0.97)",
        borderRadius:18, borderWidth:2.5, borderColor:color,
        ...
        <Text style={{fontSize:14, fontWeight:"900", color, textAlign:"center"}}>{text}</Text>
      </View>
    </Animated.View>
  );
}
...
function IdleOverlay({ onStart, onExit, sw, sh }) {
  ...
  return (
    <View style={st.overlayBg}>
      <TouchableOpacity style={st.topExit} onPress={onExit} activeOpacity={0.8}
        hitSlop={{top:14,bottom:14,left:14,right:14}}>
        <Text style={st.exitTxt}>✕</Text>
      </TouchableOpacity>
      ...
      <Text style={st.idleHint}>
        ...
        <Text style={{color:C.teal,fontWeight:"700"}}>Tap</Text> a building to fire your laser!{"\n"}
        Blast the <Text style={{color:C.green,fontWeight:"700"}}>right words</Text> — avoid the wrong ones!
      </Text>
      <Animated.View style={{transform:[{scale:pulse}]}}>
        <TouchableOpacity style={st.redBtn} onPress={onStart} activeOpacity={0.85}>
    ...
    <View style={st.overlayBg}>
      ...
      <Animated.View style={[st.card,{transform:[{translateY:slide}],opacity:fade}]}>
        ...
        <View style={st.roundBadge}>
          <Text style={st.roundBadgeText}>Mission {round.id} of {ROUNDS.length}</Text>
        </View>
        ...
        <Text style={{color:C.textSec,fontSize:11,fontWeight:"700",marginBottom:8,letterSpacing:1}}>TARGET WORDS:</Text>
        <View style={st.chipRow}>
          {round.targetWords.slice(0,8).map(w=>(
            <View key={w} style={st.chip}><Text style={st.chipText}>{w}</Text></View>
          ))}
        </View>
        <TouchableOpacity style={[st.redBtn,{width:"100%"}]} onPress={onStart} activeOpacity={0.85}>
...
export default function SuperHeroMissionGame({ onExit }) {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      for (const [ref,asset] of [
        ...
        try { const {sound}=await Audio.Sound.createAsync(asset); if(alive) ref.current=sound; else sound.unloadAsync(); } catch(_) {}
      }
    })();
  ...
  const tick = useCallback(() => {
    ...
    const rightmost = visible.length > 0
      ? Math.max(...visible.map(b => b.x + b.w))
      : sw;
    ...
    if (queueIdxRef.current >= queueRef.current.length && visible.length === 0) {
      endRound();
      return;
    }
    ...
    <View style={st.root}>
      ...
      <View style={StyleSheet.absoluteFill} pointerEvents="none">
        {Array.from({length:35},(_,i)=>(
          <View key={i} style={{
            position:"absolute",
            left:(i*113+17)%sw, top:(i*53+9)%(sh*0.45),
        ...
        {/* Moon */}
        <View style={{
          position:"absolute", top:sh*0.05, right:sw*0.08,
          width:30, height:30, borderRadius:15,
          backgroundColor:"#FFF9E3",
          shadowColor:"#FFF9E3", shadowOffset:{width:0,height:0},
          shadowOpacity:0.9, shadowRadius:16, elevation:6,
        }}/>
      </View>
      ...
      {laser && (
        <LaserBeam
          key={laser.id}
          fromX={laser.fromX} fromY={laser.fromY}
          toX={laser.toX}    toY={laser.toY}
          onArrived={() => {}}
        />
      )}
      ...
      {/* ── HUD ── */}
      {isPlaying && (
        <Hud round={round} timeLeft={timeLeft} score={score}
          onExit={handleExit} badgeScale={badgeScale}/>
      )}
      ...
      {/* ── Phase overlays ── */}
      {phase==="idle"     && <IdleOverlay onStart={handleStartGame} onExit={handleExit} sw={sw} sh={sh}/>}
      {phase==="briefing" && <MissionBriefing round={round} onStart={handleStartRound} onExit={handleExit}/>}
...
{/* ── Notification ── */}
      {notif && (
        <Notif
          key={notif.id}
          text={notif.text} color={notif.color} icon={notif.icon}
          sw={sw} sh={sh}
...
const st = StyleSheet.create({
  ...
  idleSubtitle: { fontSize:13, fontWeight:"700", color:C.yellow, letterSpacing:2, marginBottom:18, textAlign:"center" },
  ...
  classifiedHeader: {
    width:"100%", backgroundColor:"rgba(0,188,212,0.1)",
    borderRadius:10, borderWidth:1, borderColor:C.tealBorder,
    paddingVertical:10, alignItems:"center", marginBottom:12,
  },
  classifiedTxt: { fontSize:12, fontWeight:"900", color:C.teal, letterSpacing:2 },
  roundBadge: {
    backgroundColor:"rgba(255,23,68,0.15)", borderRadius:20,
    borderWidth:1, borderColor:C.redBorder,
    paddingHorizontal:14, paddingVertical:3, marginBottom:10,
  },
  ...
  redBtn: {
    backgroundColor:C.red, borderRadius:30,
  ...
  redBtnText: { fontSize:17, fontWeight:"900", color:"#FFF", letterSpacing:0.5 },
