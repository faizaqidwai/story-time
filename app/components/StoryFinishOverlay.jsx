...
const SAMPLE_WORDS = [
  "elephant", "jungle", "adventure", "river",
  "stampede", "savanna", "family", "journey",
];
    ...
    <View style={{ width: iconSize + 40, height: iconSize + 22, alignItems: "center", justifyContent: "flex-end" }}>
      {PILE_OFFSETS.map((o, i) => (
        <ExpoImage key={i} source={source} style={{
          position: "absolute",
          width: iconSize * o.sc, height: iconSize * o.sc,
          bottom: 0, left: "50%",
...
const wS = StyleSheet.create({
  chip: {
    ...
    paddingHorizontal: pad.sm, paddingVertical: pad.xs, margin: pad.xs,
  },
  text: { fontFamily: FONTS.bold, color: C.teal, fontSize: font.md },
});

// ─────────────────────────────────────────────────────────────────────────────
// SCRATCH COVER — SVG clip-path tear for the game card
// ─────────────────────────────────────────────────────────────────────────────
const CARD_W = 130;
const CARD_H = 170;

function buildTearPath(progress) {
  // progress 0 = no tear, 1 = small, 2 = medium, 3 = large (fully revealed)
  if (progress === 0) return null;

  const tearHalfW =
    progress === 1 ? CARD_W * 0.12
    : progress === 2 ? CARD_W * 0.22
    : CARD_W * 0.40;
  ...
  const innerPoly = allPts
    .map((p, i) => `${i === 0 ? "M" : "L"} ${p.x.toFixed(1)},${p.y.toFixed(1)}`)
    .join(" ") + " Z";
  return `${outerRect} ${innerPoly}`;
}

// Builds SVG path data for thin diagonal scratch mark lines inside the tear.
// These run parallel to the tear direction — short, irregular, realistic.
function buildScratchLinesPath(progress) {
  ...
  const dxC = cx2 - cx1, dyC = cy2 - cy1;
  const lineLen = Math.sqrt(dxC * dxC + dyC * dyC);
  // Unit vectors along and perpendicular to tear direction
  const ux = dxC / lineLen, uy = dyC / lineLen; // along tear
  const px = -dyC / lineLen, py = dxC / lineLen; // perpendicular

  // Tear width at this progress level — matches buildTearPath
  const tearHalfW =
    progress === 1 ? CARD_W * 0.09
    : progress === 2 ? CARD_W * 0.17
    : CARD_W * 0.32;

  // Generate short diagonal lines inside the tear area
  const lineCount = progress === 1 ? 5 : progress === 2 ? 9 : 14;
  const paths = [];

  for (let i = 0; i < lineCount; i++) {
    // Position along the center line (spread across the tear length)
    const t = 0.15 + (i / (lineCount - 1)) * 0.7;
    const midX = cx1 + dxC * t;
    const midY = cy1 + dyC * t;

    // Perpendicular offset — scatter lines across the tear width
    const perpOffset = tearHalfW * 0.55 * Math.sin(i * 1.9 + 0.4) * (i % 2 === 0 ? 1 : -0.65);
    const lx = midX + px * perpOffset;
    const ly = midY + py * perpOffset;

    // Line length — short, irregular
    const scratchLen = 10 + 8 * Math.abs(Math.sin(i * 1.3));
    const x1 = (lx - ux * scratchLen * 0.5).toFixed(1);
    const y1 = (ly - uy * scratchLen * 0.5).toFixed(1);
    const x2 = (lx + ux * scratchLen * 0.5).toFixed(1);
    const y2 = (ly + uy * scratchLen * 0.5).toFixed(1);
...
function ScratchGameCard({ slot, progressAfter }) {
  ...
  useEffect(() => {
    ...
    Animated.sequence([
      // 1. Vertical shake — slower, 3 bounces with clear rhythm
      ...
      // 2. Clear pause between shake directions
      Animated.delay(500),
      // 3. Horizontal shake — distinct from vertical, own rhythm
      ...
      // 4. Pause before scratch — let it settle
      Animated.delay(450),
      // 5. Scratch reveal — animates from 0 to progressAfter
  ...
  return (
    <View style={scrS.outerWrap}>
      ...
      <Animated.View style={[scrS.glowRing, {
        opacity: glowOpacity,
        transform: [{ scale: glowScale }],
      }]} />

      <Animated.View style={[scrS.cardWrap, {
        ...
        <View style={[scrS.card, { backgroundColor: gradient[1] }]}>
          ...
          {/* Real game animation — same as GameCard ────────────────────────── */}
          {(() => {
...
const scrS = StyleSheet.create({
  outerWrap: {
    ...
    marginBottom: pad.sm,
    // extra space for the glow ring
    padding: 12,
  },
  glowRing: {
    position: "absolute",
    width: CARD_W + 24, height: CARD_H + 24,
  ...
  circle1: {
    ...
    top: -20, right: -20, backgroundColor: "rgba(255,255,255,0.07)",
  },
  circle2: {
    position: "absolute", width: 60, height: 60, borderRadius: 30,
    bottom: 20, left: -15, backgroundColor: "rgba(255,255,255,0.05)",
  },
  questionWrap: {
    ...StyleSheet.absoluteFillObject,
    alignItems: "center", justifyContent: "center",
...
const StepCard = React.forwardRef(function StepCard(
  ...
  return (
    <Animated.View ref={ref} style={[pS.card, {
      borderColor: config.borderColor, shadowColor: config.glowColor,
      opacity: opAnim, transform: [{ scale: enterAnim }],
    }]}>
      <Text style={pS.heading}>
        {isScratchStep ? "🎮 A Surprise is Hiding!" : "🎉 Story Complete!"}
      </Text>
      ...
      {isScratchStep ? (
        ...
        <>
          <View style={pS.pileWrap}>
            <PileIcon source={config.image} size={72} glowColor={config.glowColor} />
          </View>
          <Text style={[pS.countText, { color: config.accentColor }]}>
            {config.countLabel}
          </Text>
...
const pS = StyleSheet.create({
  card: {
    ...
    padding: pad.xl, alignItems: "center",
  },
  heading: {
  ...
  subLabel: {
    fontFamily: FONTS.regular, fontSize: font.lg, color: C.textMuted,
    marginTop: pad.xs, marginBottom: pad.sm,
  },
  scratchHint: {
    fontFamily: FONTS.regular, fontSize: font.md, color: C.textMuted,
    textAlign: "center", paddingHorizontal: pad.sm, lineHeight: font.md * 1.5,
  },
  wordsWrap: {
    flexDirection: "row", flexWrap: "wrap",
    justifyContent: "center", marginVertical: pad.s, maxWidth: SW - 60,
  },
  continueBtn: {
...
const StoryFinishOverlay = ({
  ...
  // Capture scratchSlot at show time so it doesn't change mid-animation
  ...
  // Build steps array — scratch step only included if scratchSlot is set
  const buildSteps = (slot) => {
    const base = [
      {
        image: require("../../assets/img/coin.png"),
        accentColor: C.yellow, glowColor: C.yellowGlow, borderColor: C.yellowBorder,
        countLabel: `${coinsEarned} Coins`, subLabel: "coins earned! 🪙", words: null,
      },
      {
        image: require("../../assets/img/diamond.png"),
        accentColor: C.purple, glowColor: C.purpleGlow, borderColor: C.purpleBorder,
        countLabel: `${diamondsEarned} Diamonds`, subLabel: "diamonds earned! 💎", words: null,
      },
      {
        image: require("../../assets/img/bag.png"),
        accentColor: C.teal, glowColor: C.tealGlow, borderColor: C.tealBorder,
        countLabel: `${wordsCollected} Words`, subLabel: "words learned! 📚",
        words: sampleWords?.slice(0, 8) ?? SAMPLE_WORDS.slice(0, Math.min(wordsCollected, 8)),
      },
    ];
  ...
  useEffect(() => {
    if (!visible) {
      ...
      Animated.timing(sheetY, {
        toValue: SHEET_HEIGHT, duration: 350,
  ...
  useEffect(() => {
    ...
    Animated.parallel([
      ...
      if (!isLast) {
        // Scratch step holds longer so the animation can play out
  ...
  const handleContinue = useCallback(() => {
    ...
    Animated.parallel([
      Animated.timing(sheetY, {
        toValue: SHEET_HEIGHT, duration: 380,
        easing: Easing.in(Easing.quad), useNativeDriver: true,
      }),
  ...
  return (
    <Modal
      ...
      animationType="none"
      // Tapping outside calls handleContinue — user can dismiss at any time
      onRequestClose={handleContinue}
    >
      <View style={styles.shell} pointerEvents="box-none">
        {/* Tappable scrim — dismiss on outside tap */}
...
const styles = StyleSheet.create({
  ...
  scrim: {
    ...
    height: TOP_CLEAR + 20, backgroundColor: "rgba(0,0,0,0.25)",
  },
  sheet: {
  ...
  handle: {
    ...
    backgroundColor: "rgba(255,255,255,0.2)", marginBottom: pad.xs,
  },
});
