...
const TICK_MS = 16;
const GRAVITY = 0.28; // was 0.45 — slower downward pull
const TAP_IMPULSE = -7.5; // was -9.5 — gentler upward kick
const MAX_FALL_VEL = 7; // was 12  — caps terminal velocity
...
export default function FlappyWordGame({ onExit }) {
  ...
  useEffect(() => {
    ...
    return () => {
      alive = false;
      [sndButton, sndPop, sndCorrect, sndWrong, sndWin, sndLose].forEach(
        (r) => {
  ...
  // ── EXIT — works from ANY phase, no guard ─────────────────
  ...
  // ── END GAME — only transitions from "playing" ────────────
  ...
  const spawnCoins = (wx, wy) => {
    const coins = Array.from({ length: 5 }, (_, i) => ({
      ...
      startX: wx - 11,
      startY: wy - 11,
      endX: coinBadgePos.current.x,
      endY: coinBadgePos.current.y,
      delay: i * 60,
    }));
  ...
  const tick = useCallback(() => {
    ...
    // Ceiling clamp — bird bounces off top instead of dying
    ...
    // Only ground kills
    if (birdYRef.current >= GROUND_Y - BIRD_H) {
      endGame("dead");
      return;
    }
    ...
    if (tickRef.current % WORD_SPAWN_EVERY === 0) {
      ...
      if (cidx < cq.length) {
        ...
        const MIN_GAP_Y = 180; // was 95 — much more breathing room
        const avoidObjs = shuffle(theme.avoid)
          ...
          .map((a) => {
            ...
            if (Math.abs(diff) < MIN_GAP_Y) {
              // Always push the avoid word to the opposite side with extra margin
              w.y = diff >= 0 ? cWord.y + MIN_GAP_Y : cWord.y - MIN_GAP_Y - w.h;
              // Keep well inside screen bounds
              w.y = Math.max(
                STATUS_H + 140,
                Math.min(GROUND_Y - w.h - 40, w.y),
              );
            }
            return w;
          });
...
function WordTicker({ queue, currentIdx }) {
  ...
  const glowBorder = glowAnim.interpolate({
    inputRange: [0, 1],
    outputRange: ["rgba(0,188,212,0.5)", "rgba(0,188,212,1)"],
  });
  const glowBg = glowAnim.interpolate({
    inputRange: [0, 1],
    outputRange: ["rgba(0,188,212,0.15)", "rgba(0,188,212,0.35)"],
  });
        ...
        {queue.map((w, i) => {
          ...
          return (
            <View
              ...
              style={[
                styles.tickerWord,
                isPast ? styles.tickerWordPast : styles.tickerWordUpcoming,
                { width: TICKER_ITEM_W },
              ]}
            >
              <Text
                style={[
                  styles.tickerWordText,
                  isPast && styles.tickerWordTextPast,
                ]}
              >
                {isPast ? "✓ " : ""}
                {w}
...
function ResultOverlay({ phase, score, total, theme, onReplay, onExit }) {
  ...
  const msg = won
    ? `All words collected in ${theme}!`
    : `Better luck next time in ${theme}!`;
  return (
    <View style={styles.overlayBg}>
      ...
      >
        ...
        <Text style={[styles.resultTitle, { color: won ? C.green : C.red }]}>
          {title}
        </Text>
